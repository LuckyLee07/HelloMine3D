...
LOG_DIR="${2:-$ROOT_DIR/build/adventure-terrain-$(date +%Y%m%d%H%M%S)-$CONFIGURATION}"
...
shasum -a 256 "$ROOT_DIR/src/HelloMine3D/World/Generation/Terrain/AdventureTerrainPlanner.cpp" \
    ...
    "$ROOT_DIR/tools/tests/adventure_terrain_test.cpp" > "$LOG_DIR/sources-sha256.txt"
clang++ "${OPTIONS[@]}" "$ROOT_DIR/tools/tests/adventure_terrain_test.cpp" \
    "$ROOT_DIR/src/HelloMine3D/World/Generation/Terrain/AdventureTerrainPlanner.cpp" \
    -o "$LOG_DIR/adventure_terrain_test" > "$LOG_DIR/build.log" 2>&1
"$LOG_DIR/adventure_terrain_test" "$LOG_DIR/samples.csv" | tee "$LOG_DIR/test.log"
