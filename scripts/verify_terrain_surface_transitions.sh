...
LOG_DIR="${2:-$ROOT_DIR/build/terrain-transitions-$(date +%Y%m%d%H%M%S)-$CONFIGURATION}"
...
shasum -a 256 "$ROOT_DIR/src/HelloMine3D/World/Generation/Terrain/TerrainFoundation.cpp" \
 ...
 "$ROOT_DIR/tools/tests/terrain_surface_transition_test.cpp" > "$LOG_DIR/sources-sha256.txt"
clang++ "${OPTIONS[@]}" "$ROOT_DIR/tools/tests/terrain_surface_transition_test.cpp" \
 "$ROOT_DIR/src/HelloMine3D/World/Generation/Terrain/TerrainFoundation.cpp" \
 -o "$LOG_DIR/terrain_surface_transition_test" > "$LOG_DIR/build.log" 2>&1
"$LOG_DIR/terrain_surface_transition_test" "$LOG_DIR/samples.csv" | tee "$LOG_DIR/test.log"
