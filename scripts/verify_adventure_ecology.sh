...
LOG_DIR="${2:-$ROOT_DIR/build/adventure-ecology-$(date +%Y%m%d%H%M%S)-$CONFIGURATION}"
...
shasum -a 256 "$ROOT_DIR/src/HelloMine3D/World/Generation/Terrain/AdventureTerrainPlanner.cpp" \
    ...
    "$ROOT_DIR/tools/tests/adventure_ecology_test.cpp" > "$LOG_DIR/sources-sha256.txt"
clang++ "${OPTIONS[@]}" "$ROOT_DIR/tools/tests/adventure_ecology_test.cpp" \
    "$ROOT_DIR/src/HelloMine3D/World/Generation/Terrain/AdventureTerrainPlanner.cpp" \
    "$ROOT_DIR/src/HelloMine3D/World/Generation/Terrain/AdventureWaterPlanner.cpp" \
    "$ROOT_DIR/src/HelloMine3D/World/Generation/Ecology/AdventureEcologyPlanner.cpp" \
    -o "$LOG_DIR/adventure_ecology_test" > "$LOG_DIR/build.log" 2>&1
"$LOG_DIR/adventure_ecology_test" "$LOG_DIR/samples.csv" | tee "$LOG_DIR/test.log"
