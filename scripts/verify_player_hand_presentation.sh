...
LOG_DIR="${2:-$ROOT_DIR/build/player-hand-$(date +%Y%m%d%H%M%S)-$CONFIGURATION}"
if [[ -e "$LOG_DIR" ]]; then echo "Refusing to replace previous evidence: $LOG_DIR" >&2; exit 2; fi
mkdir -p "$LOG_DIR"
OPTIONS=(-std=c++17 -Wall -Wextra -Werror -I"$ROOT_DIR/src/external/glm")
...
"$LOG_DIR/player_hand_test" | tee "$LOG_DIR/test.log"
echo "[PLAYER_HAND] configuration=$CONFIGURATION logs=$LOG_DIR"
