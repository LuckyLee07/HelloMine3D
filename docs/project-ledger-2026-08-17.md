...
| Active next scope | Stage 8 has delivered K1-K4, G1 and H1, plus Q1's portable contract and Q2's portable instrumentation/fixtures. G2 is the next gameplay batch; H2-H3, Q1/Q2 Windows budgets, Q3 and G3-G6 remain. R3 still closes D2, D4 and D6 during the final acceptance batch. B3 and the native R4 ThreadSanitizer gate are Done. | Milestones K, H, Q and G below; ordering in `Recommended Order` |
...
4. **Expose the safe flow** - K4 and G1 are complete; continue with G2 so recipes become player crafting through the normal UI/input boundary.
