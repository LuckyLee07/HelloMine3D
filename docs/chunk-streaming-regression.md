...
M7 已实现安全版本：主线程发布视锥副本，worker 在视距 16 的完整目标集合上按
“视锥内优先、距离次之”排序，转向 5 度或跨垂直 section 时重排。它从不删除
视锥外目标，因此转身不会出现永久空洞。

### R3 空 section 已跳过（M6）

M6 复用每层 opaque 计数识别六面封闭 section，后台和同步路径都不再运行空 mesh
构建；边界打开后会立即恢复正常建模。参见 `docs/minigame-reference.md` 第 7 条。
