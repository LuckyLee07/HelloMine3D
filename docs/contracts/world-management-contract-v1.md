...
- `WorldCatalogue` 继续保持只读；元数据发布使用 `WorldSave`/`StorageTransaction`，
  备份恢复委托给 `WorldBackup`。
- 世界预览不进入 catalogue 条目或批量枚举。只有用户选中一个已枚举且身份重新校验的世界后，
  `WorldManagementService` 才有界读取其 `world-preview.hmp`；渲染层不直接拼路径或读存档。
...
- 预览读取最多接受 4096 B、49×25 个 8 m 格，并先拒绝 symlink、非普通文件和超限文件。
  来源新鲜度只轻量读取 `exploration.hmap` 的有界文件状态与尾部 8 字节 checksum，连同文件长度
  比对预览头，不在菜单解析整份探索地图。预览缺失、损坏、世界身份不符或 source revision
  陈旧只返回“预览不可用”，不改变 catalogue 成功结果，也不阻止打开世界。
- 成功恢复备份后，恢复前的 `world-preview.hmp` 必须失效；删除缓存或让来源修订核对拒绝它
  均可，不能展示与恢复后地表历史不一致的旧预览。
