...
1. `precise-object-edit`：以旧 FS3 母版为编辑目标，在不复制任何第三方游戏或
   纹理包资产的约束下，按既有三行语义顺序重绘为中低饱和、五级明度、1-3 像素色块、左上
...
- 记录：`docs/reports/developer-visual-record-v10b2.txt`，`result=PASS`，可由
