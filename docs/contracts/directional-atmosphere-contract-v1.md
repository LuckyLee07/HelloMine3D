...
综合视觉升级 r19 在同一有界模型内增加形体受光：两处三频噪声样本同时计算解析斜率，
不增加噪声采样、纹理、raymarch 或世界查询。斜率参与太阳/月亮方向的明暗，厚云底部较暗、
薄边较亮，沿用环境提供的昼夜色板。日月贡献按各自强度连续相加，不以强弱切换光源方向。
进入云层时采样点仍取射线入口；近零穿行距离的透明度、水平视线淡出和上下表面补光均连续
过渡，避免相机刚越过高度 156/180 时出现整片亮度突变。FS2 回退公式保持原样。
macOS GPU 检查使用实际生产 fragment，覆盖定向受光、上下边界、昼夜连续漂移和回退逐像素一致；
实机场景与性能证据见[综合视觉实施记录](../reports/visual-upgrade-2026-09-17.md)。
