...
> Architecture Lab 迁移说明（2026-08-31）：N12A-C 工程证据保持冻结；可观察的双语 UI、
> cue/字幕和音频生命周期映射到 `docs/current/ai-assisted-gameplay-acceptance-v1.md` 的 `AI-07`。
> 人类阅读偏好、听感和审美为 `NOT_CLAIMED`，原人工延期条款不再是当前待办。
...
- UI 缩放范围保持 0.75-1.75。核心窗口使用可换行文本、稳定 `##id` 和可滚动区域；翻译文本
  ...
  真实窗口可见性进入 `AI-07`，不能由自动测试伪装成 PASS，人类阅读偏好不声明。
...
- 暂停、设备挂起、切世界、回主菜单、静音、音乐音量归零、设备失败和退出都有幂等回收语义；
  后台线程不能越过音频运行时生命周期。完整边界见 `docs/contracts/streamed-music-contract-v1.md`。
...
- 自动门禁：两语 key 完全一致、非法 UTF-8/缺 key/缺字体/未知 locale、settings v0-v4 迁移、
  ...
  客户端及全量 Windows 门禁通过。
- R3 的部分开发者自测保留为历史证据，原物理模板保持 `NOT_RUN/SUPERSEDED`。当前可读性与
  音频功能进入 `AI-07`，人类主观体验 `NOT_CLAIMED`。
...
- VS2017/v141 Debug/Release 客户端 0 错误；双配置世界 675/675、目录 59/59、资源包 32/32，
  ...
  进世界两场比较均 `PASS`。
- 双语界面、截断/重叠、字幕和音频生命周期进入 `AI-07`，当前为 `NOT_RUN`；人类阅读偏好、
  听感和审美为 `NOT_CLAIMED`。N12A 工程证据保持 PASS，不由本迁移改写。
...
- 同身份隐藏启动的可用菜单为 `711.836/712.408 ms`，进入可控世界为
  `398.068/416.950 ms`，两项比较均 `PASS`。cue/字幕/生命周期进入 `AI-07`；人类听感为
  `NOT_CLAIMED`，真实无设备机器按未来明确批准的目标环境验证。
...
- Windows 真实三帧隐藏启动报告 `windows-waveout-stream real=1`、1 条曲目、1,764,000 字节 PCM、
  `degraded=0` 并退出 0。真实暂停/切世界/静音/退出功能进入 `AI-07`，人类听感为
  `NOT_CLAIMED`；真实无设备机器按未来明确批准的目标环境验证。
