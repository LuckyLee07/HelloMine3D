...
Waystone 共鸣机制、保存边界、双配置构建和自动回归已关闭；不看名称的动态辨识、危险窗口观感、
掉落价值感与完整守护战真人体验保持 `Deferred`，不登记为自动 PASS。
...
2. 实际观察 Idle、Chase、Windup、Recover、受击和死亡姿态，记录攻击窗口是否比旧单立方体清晰，
   以及是否存在抖动、穿插或不舒适反馈。
3. 用每类自然敌人的正式掉落完成一次采集/恢复/制作计划，判断身份奖励是否值得改变路线。
4. 完整走通一次 Waystone 守护战，至少成功用共鸣打断一次 Windup，并记录误触、冷却判断、死亡原因
   和策略变化。
5. 在活动桌面恢复后补采适用 Q1；macOS 原生窗口继续保持 `Verify`。

这些项目统一后置，不阻塞 P11F 的可自动开发部分，也不能在未执行时写成 PASS。
