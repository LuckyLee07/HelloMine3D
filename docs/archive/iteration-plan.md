...
> 验收模型更新（2026-08-31）：本文历史阶段中的“真人/人工/Physical Input/Deferred”描述
> 保留当时语境，不再是当前待办。当前功能可玩性使用
> `docs/current/ai-assisted-gameplay-acceptance-v1.md` 的 AI/Computer Use 场景；人类主观与物理体验
> 为 `NOT_CLAIMED`，历史物理合同为当前门槛的 `SUPERSEDED`。
...
| 真实窗口功能证据 | OIS 到控制器的数据边界已自动验证；D2/D4/D6/R3 的 Windows 功能路径映射到 `AI-01..AI-04`，当前为 `NOT_RUN`。原 R3/Physical Input 保持历史 `NOT_RUN`，物理手感不声明。 |
| 当前路线状态 | 第 8 阶段、Stage 9/N7-N12、BETA-RC、Stage 10/VISUAL-RC 与 Stage 11/P11-0-P11F Windows 自动工程均已完成。当前没有剩余 Stage 11 代码批次；Architecture Lab 是候选长期路线，其能力目录不自动成为 backlog。 |

## 迭代阶段

下列表格保留最初的架构拆分和优先级，不再直接表示未完成工作。当前交付映射
如下；其中未进入 `docs/current/todolist.md` 的 P2/产品化候选不计作当前待办。
...
| 第 7 阶段 | Engineering Done；AI `NOT_RUN` | D1-D6、R1-R2、R4-R5、X1-X3 已实现并有自动/硬件/TSan 证据；D2/D4/D6 功能路径由 `AI-02..AI-04` 承接。 |
| 第 8 阶段 | 工程闭环；AI `NOT_RUN` | `K1-K4`、`G1-G6`、`N1-N6`、`H1-H3` 和 `Q1-Q3` 已完成；完整正常旅程进入 `AI-08`。 |
| 第 9 阶段 | 工程 Done | `N7-N12` 与 `BETA-RC` 自动门禁、正式 Q1/Q3 和发行证据已封板；功能/表现分别进入 AI 交互与视觉场景。 |
| 第 10 阶段 | Done（Windows 工程） | `V10A`、`V10B1-B3`、`V10C-V10E` 与 `VISUAL-RC` 已封板；AI 产品表现 `NOT_RUN`，人类主观体验 `NOT_CLAIMED`，新 macOS 运行按未来里程碑批准。 |
| 第 11 阶段 | Engineering Done；AI `NOT_RUN` | P11-0/P11A/P11B/P11-1/P11C/P11D/P11-2/P11E/P11F 工程均完成；映射到 `AI-01..AI-08`，不再保留真人延期清单。 |
| Architecture Lab | Proposed | 路线文档已建立；每个 Sprint 要有游戏内 Demo，每个 Track 要用干净包完成 `AI-08`，只有写入 `todolist.md` 的下一批才获批。 |
...
任务状态和逐项验收仍以 `docs/current/todolist.md` 为准。
...
1. `R3` v1/Physical Input v2 保留为历史 `NOT_RUN/SUPERSEDED` 物理协议；D2、D4、D6 的
   可观察功能现由 `AI-01..AI-04` 在目标 Release 构建上验证，当前为 `NOT_RUN`。
...
| ------ | -------- | -------- | ------------ |
| 长期存档 | `K1-K4` | 建立稳定世界目录、事务保存、有界备份、损坏恢复和世界管理 UI。 | 创建、保存、崩溃中断、恢复、重命名和可恢复删除均有自动与 AI/Computer Use 证据；任何失败不得覆盖最后一份已验证存档。 |
...
| 1：世界入口（Done） | `K4` | 世界管理服务、列表模型、最近游玩、创建/打开/重命名、备份选择、恢复和可恢复删除，再接入主菜单。 | 44 项 K1/K4 断言、Debug 主程序编译、隐藏主菜单和直进世界冒烟通过；正常窗口功能现映射到 `AI-01/AI-02`。 | 玩家路径和失败边界已落地；id/目录稳定，显示名可变，删除可恢复。 |
...
| 14：Release Candidate（Engineering Done；AI `NOT_RUN`） | AI/Computer Use、AI Visual | H2-H3、Q1-Q3、R5 干净包和最终双配置自动门禁已完成；R3 v1/Physical Input v2 仅保留为历史物理基线。 | `AI-01..AI-08` 按现行规范留证；未执行不写 PASS，主观体验为 `NOT_CLAIMED`。 | N1-N6 已冻结，只修缺陷。 |
...
5. 最终 Windows 双配置构建、存档故障、资源包、崩溃、性能、soak 和干净包自动门禁已
   通过；后续用 `AI-01..AI-08` 分开验证功能与可观察表现，不能用 R3 v1 或自动预检替代
   正常窗口证据。封板批只修缺陷，不继续增加内容。
...
| N6 产品体验（Done） | 键位、辅助选项、UI 统一、配方/目标可读性、基础程序化音效反馈与声音字幕；正式采样音效和低密度音乐后来独立交付。 | 已交付；当前功能/表现分别进入 AI 交互和 AI 视觉场景，历史人工合同保留审计用途。 |
...
RC0 已被 BETA-RC 取代；R3 v1 保留为历史基线，D2/D4/D6/R3 功能转入 AI 场景且当前为
`NOT_RUN`。Stage 9 工程随后转入 Stage 10。
...
| 2 | FS3 体素美术与 HUD（Done） | 一致图集、图标化快捷栏、手持物与基础交互反馈。 | 539 项双配置世界回归、27 项资源包、47 项清单、来源记录、隐藏截图与焦点守卫通过；功能/表现由 AI 验收规范承接。 |
| 3 | RC0 基线收口（Superseded） | bundle、文档一致性和自动基线已由 BETA-RC 覆盖。 | 不再作为活跃批次或标签入口；历史人工合同不再是当前门槛。 |
...
| 13 | N12A 本地化完成（Done） | 已完成中英文、字体、长文本、语义字幕、Credits 和许可。 | 341 个双语 key、675 项世界、32 项资源包、51 项清单、隐藏客户端和完整门禁通过；双语可见性进入 `AI-07`。 |
| 14 | N12B 正式采样音效（Done） | 已建立采样音效 schema/后端并替换程序化占位声音。 | 9 个 WAV、312,230 字节缓存、双配置 681 项世界/34 项资源包、隐藏真实后端和完整门禁通过；cue/字幕功能进入 `AI-07`。 |
| 15 | N12C 低密度音乐（Done） | 已交付单通道流式环境音乐及暂停、淡入淡出、降级和清理。 | settings v4、原创 20 秒 WAV、64 项清单、699 项世界、38 项资源包、真实 WaveOut、完整门禁和同身份 Q1 通过；人类听感不声明。 |
| 16 | BETA-RC（工程 Done） | 已重建迁移、性能、长稳、崩溃、自动输入预检和发行包基线。 | 六类 Q1 与两档 1800 秒 Q3 通过；bundle 已验证；历史人工状态见冻结报告，当前由 AI 规范承接。 |
...
非目标、最低回归和退出条件统一见 `docs/archive/beta-gameplay-roadmap.md`；FS1-FS3 的细化合同见
...
明确兼容许可证。权威合同见 `docs/archive/visual-quality-roadmap.md`。
...
| 2 | V10B1 材质与图集管线（Done；Windows） | M | 参数化图集/tile/颜色管线。 | 旧图集静态前景像素兼容、shader/资源启动负例和 Windows 双配置通过；新 macOS 运行 `NOT_RUN`。 |
...
| 5 | V10C 定向大气与立体云（Done；Windows） | M/L | terrain/water/actor/sky 共享定向雾，有界云层提供视差、绝对时间移动和上下表面。 | 双配置聚焦 21/21、Release 世界 741/741、十图多帧视觉 PASS；所有者批准快速流送单样本可见延迟例外。 |
| 6 | V10D 可选方向阴影（Done；Windows） | L | 单近景太阳 shadow map 与 Off/Medium/High 档位已经落地。 | settings v5、能力回退、双配置回归、同场景各档性能和六图视觉 PASS；最终无需性能例外。 |
| 7 | V10E 轻量后处理（Done；Windows） | M/L | 可关闭 tone curve、确定性抖动和八采样极轻 bloom 已落地。 | settings v6、HUD 排除、能力回退、双配置/资源/失败负例、同档性能与六图视觉 PASS。 |
| 8 | VISUAL-RC（Done：Windows 工程） | M | before/after、开发者检查、渲染身份、性能、许可证、干净包和 bundle 封板。 | Windows 全门禁、正式 Q1/Q3、资源/许可、97 文件包和 Xcode 静态工程图通过；AI 产品表现进入 `AI-07`，人类主观体验 `NOT_CLAIMED`。 |
...
前 30 分钟的选择、探索奖励、地形轮廓和战斗表现。九个工程批次均已完成；原人工退出条件
现按 `docs/current/ai-assisted-gameplay-acceptance-v1.md` 映射为 AI 场景和 `NOT_CLAIMED` 边界。

| 顺序 | 批次 | 规模 | 核心结果 | 关键护栏 |
| ---- | ---- | ---- | -------- | -------- |
| 1 | P11-0 世界光源（Engineering Done；AI `NOT_RUN`） | M | 可制造火把、Rose 修正、metadata 熔炉发射和真正翻转时的局部重光照。 | 工程/视觉/Q1 已完成；真实制作与照明进入 `AI-05/AI-07`。 |
| 2 | P11A 核心操作手感（Engineering Done；AI `NOT_RUN`） | M/L | 分离使用/放置/格挡，补鼠标重绑定和冲刺/潜行模式，统一灵敏度与焦点/捕获恢复。 | settings v7、迁移、双配置与自动回归完成；功能进入 `AI-01..AI-04`，物理手感不声明。 |
| 3 | P11B 动作反馈（Engineering Done；AI `NOT_RUN`） | M | 手/工具、裂纹、粒子、命中/受击/冷却、拾取和音频微变体已实现。 | 自动时序、上限和关闭回退通过；动态辨识进入 `AI-05/AI-07`。 |
| 4 | P11-1 建造材料与工具（Engineering Done；AI `NOT_RUN`） | M/L | 木板、圆石、单方块门和 axe/shovel 的最小建造与工具选择已实现。 | 双配置与自动门禁通过；真实建造进入 `AI-05`。 |
| 5 | P11C 前 30 分钟（Engineering Done；AI `NOT_RUN`） | M/L | 成长、建造、探索最多三项并行机会、独立进度和随材料展开的配方书已实现。 | 工程回归通过；无上下文 30 分钟 AI 盲玩进入 `AI-06`，人类留存/乐趣不声明。 |
| 6 | P11D 探索奖励（Engineering Done；AI `NOT_RUN`） | L | 遗迹罗盘、营地护符和既有路标核心形成独特能力结果。 | save v12/奖励 v1 与旧世界 v0 已冻结；能力使用进入 `AI-05`，主观价值不声明。 |
| 7 | P11-2 地形轮廓与洞口（Engineering Done；AI `NOT_RUN`） | L | terrain v4 已提供 Mountain 高度域、Stone 高峰和可发现天然洞口，v1-v3 输出冻结。 | 完整回归与正式 Q1 通过；寻找/通行和视觉进入 `AI-05/AI-07`。 |
| 8 | P11E 敌人表现（Engineering Done；AI `NOT_RUN`） | L | 6-7 部件轮廓、玩法状态关键姿态、身份化掉落、死亡表现隔离和 Waystone 共鸣已实现。 | 完整工程门禁通过；战斗/共鸣/辨识进入 `AI-05/AI-07`，人类战斗乐趣不声明。 |
| 9 | P11F PLAYABILITY-RC（Engineering Done；AI `NOT_RUN`） | M | 已汇总自动门禁、迁移、性能、恢复和包证据。 | VS2017/v141 双配置、832/832 世界、六类 Q1、双档 Q3、崩溃/符号链路与 104 项干净包通过；完整载体进入 `AI-08`。 |

执行顺序固定为 `P11-0 -> P11A -> P11B -> P11-1 -> P11C -> P11D -> P11-2 -> P11E -> P11F`，
独立合同和定向回归均已完成。当前没有下一项 Stage 11 代码批次；AI 场景未运行前保持
`NOT_RUN`，主观范围保持 `NOT_CLAIMED`。
...
| 检查 | 命令/方式 |
| ---- | --------- |
| Windows 双配置完整门禁 | `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\verify_build.ps1 -VisualStudioVersion 2017`；无活动桌面时显式加 `-SkipRealWindow`，真实窗口相关结果记为 `NOT_RUN`。 |
...
| AI/Computer Use | 按 `docs/current/ai-assisted-gameplay-acceptance-v1.md` 从干净 Release 包运行 `AI-01..AI-08`；禁止 fixture、传送、注入和存档编辑。 |
...
| 历史物理输入 | R3 v1/Physical Input v2 模板保持 `NOT_RUN/SUPERSEDED`，不得由 AI 填写物理 PASS。 |
| AI 视觉/可读性 | `AI-07` 检查原尺寸截图、多帧/视频、双语 UI 和可访问音频功能；人类主观体验不声明。 |
| 干净目录发布验收 | `tools/package_windows_release.ps1`；从隔离根目录完成 manifest 预检、validation-only、真实窗口和负向校验；跳过真实窗口时结果写 `NOT_RUN`。 |
...
| Beta 批次回归（第 9 阶段） | 按 `docs/archive/beta-gameplay-roadmap.md` 的触发矩阵选择迁移、确定性、内容规模、短时 soak、R3 和发行证据；自动化 EXE 保持隐藏运行。 |
...
| M5 可玩与发布探索 | Engineering Done；AI `NOT_RUN` | D/R/X 当前范围均已实现；D2/D4/D6/R3 功能由 `AI-01..AI-04` 承接。 |
| M6 可持续游玩与可靠发布 | 工程闭环；AI `NOT_RUN` | K1-K4、G1-G6、N1-N6、H1-H3、Q1-Q3 已完成；完整流程由 `AI-08` 承接。 |
