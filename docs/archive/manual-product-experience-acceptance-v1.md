...
> Architecture Lab 状态（2026-08-31）：本文作为历史人工产品体验合同保留。既有开发者视觉
> PASS 记录继续有效；后续可观察的视觉、双语和音频功能改由
> `docs/current/ai-assisted-gameplay-acceptance-v1.md` 的 AI 交互/视觉流程覆盖。人类审美、听感、乐趣
> 和舒适度不再无限延期，统一为 `NOT_CLAIMED`。

本文把视觉风格、双语可读性和听感从 R3 物理输入协议中拆出。它定义了历史开发期间关闭单个
视觉批次的轻量记录，以及原产品发行定位下的正式产品体验记录；本文建立合同，不表示任何
尚未执行的检查已经 `PASS`。
...
- `docs/archive/manual-input-acceptance-v1.md` 只保留十二项历史物理输入基线；已定义的
  `docs/archive/physical-input-acceptance-v2.md` 负责真实键鼠、窗口焦点、容器、战斗、死亡/重生、作物和保存重启。
...
只检查结构，关闭批次必须改用 `-RequirePass`。已关闭的 V10B2/V10B3/V10C/V10D/V10E 记录
位于对应的 `docs/reports/developer-visual-record-<batch>.txt`；V10C 使用十张 Release 原图及正午多帧
序列，不以单帧替代运动检查，V10D/V10E 分别冻结阴影档位与后处理开关的最终原图。
