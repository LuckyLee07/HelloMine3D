...
本文把 BETA-RC 之后的画面升级拆成可独立实现、验收和回滚的批次。它基于
公开通用的图形算法和本项目需求，不复制第三方项目的美术资产，也不把
视觉升级扩大成渲染后端重写。
...
> `docs/current/ai-assisted-gameplay-acceptance-v1.md` 的 `AI-07`，人类审美与舒适度为
...
- 禁止复制第三方游戏的纹理、模型、声音、UI、截图裁片或标志性角色；所有正式素材
...
与听感属于独立产品体验证据，记录格式见 `docs/archive/manual-product-experience-acceptance-v1.md`。每个
视觉批次按下面三层记录：
...
实现合同见 `docs/contracts/vertex-lighting-contract-v1.md`。四角光照/AO、greedy 重建保护、边角 dirty
...
实现合同见 `docs/contracts/terrain-material-profile-contract-v1.md`。严格 v1 profile、CPU/GPU 半像素
...
图像哈希和结论见 `docs/contracts/material-visual-contract-v1.md`。
...
`docs/contracts/ecology-appearance-contract-v1.md`；该批关闭后进入 V10C。
...
批次继承。完整证据见 `docs/contracts/directional-atmosphere-contract-v1.md` 和
`docs/reports/developer-visual-record-v10c.txt`；随后完成的 V10D 不继承该例外，macOS 子状态保持
`Verify`。
...
合同与视觉记录见 `docs/contracts/directional-shadow-contract-v1.md`、
`docs/reports/developer-visual-record-v10d.txt`。Windows 状态为 `Done`，macOS 保持 `Verify`；最终
...
按原尺寸检查，记录为 `PASS`。完整证据见 `docs/contracts/post-processing-contract-v1.md`。Windows 状态
...
`docs/reports/visual-release-candidate-report-2026-08-28.md`。VISUAL-RC 标记为 `Done（Windows 工程）`；
...
Stage 11 可玩性与操作体验路线。后续内容与视觉复核又确认可制造光源、最小建造材料和地形轮廓
是原有体验退出条件的前置内容，因此插入 P11-0/P11-1/P11-2；当前只立项 P11-0，后续八批按
证据门依次排队，权威范围见 `docs/archive/playability-experience-roadmap.md`。P11-2 只改变地形生成身份
与轮廓，不重开 Stage 10 渲染管线；强制饥饿、批量新增浅层内容和成就/统计仍不进入当前范围。
