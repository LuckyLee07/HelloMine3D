...
> `docs/current/todolist.md`；验证方式和覆盖范围见 `docs/current/runtime-validation.md`。
...
| S0.3 | Done | Stabilize background chunk loading. | Worker consumes atomics, never a `Camera&`. V5 adds 240 load-center changes with concurrent block reads and state invariants, then proves mesh progress resumes at a new stable center. Client runs also reached 289 loaded chunks / 449 mesh rebuilds (`bin/perf_baseline_20260807190255313-41064`). Formal data-race tooling is still unavailable on this MSVC toolchain. |
...
| S5.3 | Done | Add `PlayerActor` and `PlayerController`. | `PlayerInputState` feeds the same live Ogre/OIS collection path. `applyInput()` samples idempotent held state and `applyMovement()` consumes it once per fixed tick; `V2/*` asserts target-velocity movement, normalized diagonals, flying jump, held sneak, interpolation, look delta and hotbar selection without moving the real mouse. |
...
in `docs/current/todolist.md` without carrying a foundation validation gap.
