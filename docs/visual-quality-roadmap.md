...
| 3 | V10B2 原创材质资产 | P0 | L | Implemented（前台视觉 Verify） | 世界、HUD 和手持物使用来源明确、像素密度与分面一致的原创材质。 |
...
遗迹、营地三张隐藏 Release 图未见接缝、黑材质或透明漏色。完整实现与哈希见
`docs/material-visual-contract-v1.md`。由于可见 Ogre 窗口创建可能抢占用户焦点，自动化不再自行
打开前台窗口；项目所有者自行连续观察 5-10 分钟并记录 PASS 前，本批保持
`Implemented（前台视觉 Verify）`，V10B3 不启动。
