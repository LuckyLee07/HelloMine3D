...
| ---- | ---- | ------ | ---- | ---- | ------------ |
| 1 | V10A 顶点平滑光照与 AO | P0 | M/L | In Progress | 方块接缝、墙角、洞穴、树冠和结构落地位置具有稳定层次，不再只有整面明暗。 |
| 2 | V10B1 材质与图集管线 | P0 | M | Planned | 图集尺寸、tile 尺寸和颜色管线不再由 shader 魔数隐式决定，错误资源能够明确失败。 |
...
实现合同见 `docs/vertex-lighting-contract-v1.md`。当前代码与自动夹具已开始落地，正式性能、
截图、短 Q3 和开发者视觉检查完成前保持 `In Progress`。
...
3. 跨 section/chunk 边界使用现有 18x18x18 快照，同一世界顶点从任一相邻 section 计算必须
   得到相同结果，不产生亮缝或暗缝。
4. 复用现有每顶点 `float light` 属性和 32 字节 terrain vertex stride，只把每面四个相同值改为
   四个独立值；不得为 AO 新增第二套顶点流或升级顶点格式。
