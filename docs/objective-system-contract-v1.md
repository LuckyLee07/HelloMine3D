...
第一版支持 `obtain_item`、`craft_item`、`place_block`、`break_block`、
...
注册表在 Ogre 初始化前从有效资源视图冻结一次。基础清单当前包含 41 项，其中目标资源 1 项。
资源包 v1 不允许取得 `objective` 类别所有权，避免未版本化覆盖改变已有世界进度语义；无窗口
测试可以从同一基础文件建立只读注册表，不维护第二份硬编码定义。

## 运行时语义

基础链保留 G6 的 10 个可见必做目标，并增加 1 个隐藏可选到达位置目标。事件目标只消费
...
HUD 显示当前目标、整数进度、下一目标标题和 3 秒完成反馈。全部 10 个必做目标完成后保留
明确的首轮会话终点。N1 不发放任何物品奖励；提示和进度永远不能修改业务状态。
