...
本方案承接本轮已交付的“暖野第一版”和本项目现有画面问题分析，供后续直接分批实施。
...
2026-09-13 用户以封面与 R1 真实试玩图再次指出树冠观感不足，并要求重新出方案。
[R1 封面观感树冠方案](r1-canopy-cover-style-plan-2026-09-13.md)是当前新的树冠设计建议；
后续用户否定斜切树冠并明确要求完整方块树冠，R1 范围据此修订为
原创方像素叶面与新世界版本化橡树轮廓；旧存档 v1–v5 保留原生成结果。此修订
以 [R1 合同](../contracts/warm-wilderness-m1-contract-v2.md) 和当前验收记录为准。
本 v2 文档的旧叶簇试验不因新方案自动恢复。当前方块橡树方案已实施，
用户认可树冠观感并授权提交一个本地版本检查点；正常持续输入证据仍缺，
R1 整轮未完成，见[方块橡树验收记录](../reports/r1-voxel-oak-acceptance-2026-09-13.md)。
...
当前 Ogre/GL3Plus 可以承担这条路线；无需以换引擎或大型后处理系统作为前置条件。
...
红黄叶可以作为局部季相式点缀；不改变整体克制的色彩方向，也不引入季节模拟。
