...
r16 已将基础水介质识别从增强大气开关分离，修复回退模式的水下亮带；云层、水面细节及定向光照继续回退。双配置客户端构建通过，新增 582 帧、与复用基线共 776 帧成对核验，21 张原图已审阅；水下昼夜、干燥岸边和正常大气检查通过限定范围视觉验收。12 次三轮性能共 69,354 帧已采完并恢复设置，但末条结束时间与合盖睡眠同秒，独立审计失败，不声明性能通过；机器当前合盖，新恢复入口在开盖前拒绝启动。运行时代码按小批次本地保存，候选交付仍为已验 r15，当前正常玩法与整体 Goal 均未完成。
