...
当前版本。地图三档范围引入 runtime settings v10，旧设置自动迁移，world save v12 与 terrain v12 不变。
具体实现、各检查结果、失败和恢复入口见[执行记录](../reports/visual-upgrade-2026-09-17.md)。
本轮已将代码、工具、文档分批本地提交；同架构 v12 的 r2 对照 72 次、12 组性能通过，P95/P99 最大约 1.0273×/1.0363×。
新增顺岸风驱水面漂移已单独提交 `864c86e`：双配置世界各 1190/1190、资源各 112/112、GPU 与窗口冒烟通过；r5 前台性能完成 18/72 次、三组通过后按用户要求停止。r7 已补水岸开启/关闭及水下共 192 帧后台检查，正式后台性能 72/72 次、12/12 组及独立审计通过，P95/P99 最大 1.0931×/1.0513×（门槛 1.10）。
独立验收已通过专用 r4 包完成正常菜单、代表性双语三字号布局、设置、地图范围及保存重开；持续移动/转向输入无效，采集/工具/容器/地标/连续动作仍 BLOCKED。
原工作包保留 r3，专用诊断包已刷新到 r6：修复 macOS 隐藏窗口创建仍激活应用的问题；Debug/Release 构建及 Cocoa 输入回归通过，direct/open 两路隐藏采集未观察到前台激活或可见窗口，设置与测试存档保留。
自动验收默认后台，旧包缺少能力标记时拒绝启动；未经用户后续明确允许不进行前台/CUA 操作，后台证据不混入前台性能或正常玩法。
专用 r7 又修复表现时钟大增量/长会话精度：双配置计时各 5/5、物品各 8/8、地图各 10/10 通过；Full/Reduced/Off 共 96 帧后台动作检查通过。随后完成 72 次后台性能对照与独立审计，实际 2560×1440，关闭截图读回；r7 工作包和正常配置均已核对恢复。掉落物四组 128 帧审阅发现薄物品侧转消失，r8 已修复：双配置客户端构建与物品各 13/13 通过，同倍率修复前后 192 帧核对及限定范围视觉检查通过；r9 已完成 24 次专项采样，但两条 High 静态记录分别出现约 25 秒、925 秒渲染停顿，独立审计判定时长协议失败；另有一条 delta 异常待查，不声明专项性能通过。工作包及正常配置已核对恢复。历史 Reduced 停摆根因尚未证实，完整正常动作与最终整合交付待验。
r10 已增加贴水陆地的静态湿润过渡：双配置定向各 29/29、完整世界各 1211/1211 通过；四组前后共 16 张隐藏画面已核对，工作包及正常配置已恢复。岸面性能与正常改岸/游泳待验，计时异常继续独立定位。
r11 已复现并修复 POSIX 墙钟回拨导致的无符号帧增量溢出：改用单调时钟，双配置故障注入各 10/10、客户端重建和两次隐藏 30 秒计时集成检查通过。后续电源日志确认两条长停顿横跨 27/927 秒系统睡眠，分类为受睡眠中断的无效样本；完整对照需在临时防系统睡眠条件下补齐，不把计时修复或短样本冒充三轮性能通过。
当前 r11 完整 72 次后台对照、12/12 组及独立审计已通过，P95/P99 最大 1.0712×/1.0555×，门槛 1.10；同计时掉落物专项 24 次、4/4 组及独立审计亦通过，最大 1.0095×/1.0289×。两版共用原生窗口/计时修复与 17 份依赖，实际 1280×720，无截图读回，电源记录未发现睡眠覆盖样本。
r12 修复大字号合成网格被挤出视区，已提交 `1feb4d5`：配方/材料与网格分列，底部按钮固定，双配置构建及十组中英文/字号/窗口布局原图检查通过。专用工作包与正常配置已恢复，目前没有采样 runner；性能证据保留 r11 身份，r12 仅修改打开的合成面板布局。干净候选客户端、说明与四组原图对照位于 `build/visual-upgrade-20260917/delivery-r12/`，111 项分发文件、324 份源码与无窗口启动校验通过。连续行进观察和当前完整正常玩法仍待最终验收，详见上述执行记录的当前恢复入口。
r13 增加仅供隐藏截图的有界平滑相机，客户端双配置构建、定向各 21 项、实际启动负例与关闭路径通过。
11 段共 1,067 帧覆盖林下、林缘、草甸、水岸、水中出水、山麓/营地、遗迹与夜间，已查看 39 张序列原图并生成 11 段复核视频。
发现出水时横向色块与明显亮度跳变，r14 已修正近裁剪表层与硬切换：实际 GPU 8,004 个跨水线样本最大相邻合成变化从 80/255 降至 2/255；资源双配置各 112/112，八段有效连续画面与 27 张关键帧检查通过；24 次三轮性能对照及审计通过，P95/P99 最大比值为 1.0122/1.0174（门槛 1.10）。
水下远端亮边已定位到天空背景未随水介质改变：r15 初版已消除开启大气时的亮边，但八段 776 帧复核发现白天出水 ROI 最大变化从 19.87 升到 48.05/255，该动态迭代未通过。
r15 修正版将共同介质过渡扩展至水面下 0.05–1.0 m，保持最多两次驻留查询；双配置客户端构建及 V10C 各 28/28 通过。四段 388 帧隐藏采集、与基线共 776 帧核验及 18 张原图审阅完成；白天出水 ROI 最大变化为 21.36/255，低于失败初版但略高于 r14，不作为舒适度门槛。开启大气时远景亮带消除，关闭大气的旧亮带仍列为未解决项。36 次、6 组定向性能及独立审计通过，174,097 帧，P95/P99 最大比值 1.006863 / 1.008316，门槛 1.10；已提交 `b1a4a1c`。
用户明确表示观察结束后，已通过菜单保存并退出预览客户端；普通预览存档和 r12 交付包保留。专用工作包恢复隐藏/noActivate 自动验证，后续自动启动不抢前台。
当前候选更新到 `build/visual-upgrade-20260917/delivery-r15/`：111 项分发文件、325 份源码与无窗口启动校验通过，附六组版本明确的原图对照。自动/GPU/相机诊断不替代完整正常玩法，Goal 尚未完成；持续键鼠输入验收仍受现有 CUA 接口能力限制，保留完整待验项，不重复无变化尝试。
r16 已将基础水介质识别从增强大气开关分离，修复回退模式的水下亮带；云层、水面细节及定向光照继续回退。双配置客户端构建通过，新增 582 帧、与复用基线共 776 帧成对核验，21 张原图已审阅；水下昼夜、干燥岸边和正常大气检查通过限定范围视觉验收。12 次三轮性能共 69,354 帧已采完并恢复设置，但末条结束时间与合盖睡眠同秒，独立审计失败，不声明性能通过；后续确认合盖但图形能力可用且不触发睡眠，改以实际图形/AC/无强制合盖睡眠门控，保留旧失败。r3 完整复验 12 次、2 组、68,855 帧及审计通过，P95/P99 最大 1.006654/0.997465；当前候选为 delivery-r16，111 项文件、325 份源码及无窗口校验通过，附八组原图。用户允许可视化窗口后，完成当前版正常建档、双语/字号设置、合成布局及保存重开的有限自测；实现者同上下文不冒充独立验收。r17 已修复合成面板遮挡预警字幕：通知统一前景绘制，双配置构建、正常双语合成界面及六组小窗口布局检查通过，已提交 `95d57c7` 并更新 delivery-r17 候选，111 项文件/325 份源码与无窗口校验通过。正常 A→B→A 切换地表图刷新完成有限自测；持续输入、地标/改块和完整正常玩法仍待验，Goal 未完成。

r17 后续补齐通知绘制专项性能：林地打开合成面板，Medium 阴影、关闭后处理，实际 2× 2560×1440 隐藏窗口；静止与四次传送流送各三轮，共 12 次、57,709 帧，逐帧及无睡眠覆盖审计通过。P95/P99 三轮中位数比分别为静止 `0.988568 / 0.993128`、流送 `0.982460 / 0.992308`，均满足 1.10 门槛。普通设置已恢复；delivery-r17 原包不变，新增性能补验收据。此项不替代仍缺少的正常连续输入验收。

r19 按用户要求继续视觉迭代：云团增加日月方向受光、云底/薄边层次，修复进入/离开云层的亮度突变。
生产 GLSL 21/21 与故障负例、资源双配置各 112/112、六组最终客户端原图检查通过；三档画质静止/流送
36 次、169,165 帧及独立审计通过，P95/P99 最大比值 1.022548/1.034386。只改 shader，复用 r17 二进制；
候选 `delivery-r19` 的 111 文件/325 源码与无窗口校验通过。持续输入验收按用户最新要求暂缓；
后续优先转向玩家与生物模型、持物连接和动作，动物表现亦在用户要求的视觉范围内。

r20 已补第一人称手臂、袖口和空手/握图标/托方块姿态，手与物共用动作，增加昼夜曝光和窄窗口适配。
客户端双配置、手部各 8/8、输入/反馈/物品定向双配置通过；14 组静态、4 组各 40 个连续时间点及
21 张最终原图审阅通过限定范围检查。空手/持剑的静止和流送共 24 次、101,897 帧及逐帧审计通过，
P95/P99 最大比值 1.007543/1.015072，门槛 1.10；普通设置已恢复，`delivery-r20` 的 111 文件/326 源码及无窗口校验通过。

r21 已修现有生物的肩髋支点、头口共同颈部和斜向步态，并补足 Stalker 静态肢体连接。
客户端/WorldRuntimeSmoke 双配置构建、P11E 各 35/35、关节步态各 14/14、三项故障注入和六项入口负例通过；
七组静态捕获、两段各 41 个连续时间点，实际审阅 11 张最终原图及一张基线原图。
四类展示模型的静止/流送共 12 次、43,636 帧及逐帧审计通过，P95/P99 最大比值 1.011866/1.033812；
`delivery-r21` 的 111 文件/326 源码与无窗口校验通过，普通配置已恢复。下一批补生物面部和材质层次。
攻击姿态混合、脚掌贴地、第三人称全身玩家、独立动物体系与暂缓的正常持续输入均未计作完成。
...
对比，现进入候选 21：参考 NMiniGame 的像素地图缓存思路，在右上角显示约 128 米地表、
