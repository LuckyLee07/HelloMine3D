...
| 6 导航与生物 | 三档地图、当前区域、已访问地标；r17 正常切世界刷新；r21 肩髋/头口支点与斜向步态修正、12 次性能通过；r22 面部和材质分区、24 次性能与候选交付完成 | 正常改块/移动/地标生命周期与完整战斗待验；四足抬头时颈部连接面积偏小，下一批修几何。独立动物体系尚未实现。 |
| 7 整合与交付 | r11 全矩阵 72 次及掉落物 24 次审计通过；后续按受影响范围逐批性能与候选交付，最近完整候选为 `delivery-r22` | 最终版本正常创建/进入、移动、采集、制作、工具、容器、战斗、设置、保存重开尚未全部验收，Goal 未完成。 |

本批 r22：双配置客户端构建、GPU 90/90 和旧 shader 负例、资源各 112/112、P11E 各 35/35、资产 80/80、
六项距离入口负例通过；16 组静态捕获与两段各 41 个连续时间点已取得，实际审阅 11 张最终原图。
Off/Medium 阴影的静止/流送共 24 次、109,124 帧及逐帧审计通过，P95/P99 最大比值 1.009307/1.032834；
`delivery-r22` 的 111 项文件、326 份源码、两个 shader 内容与无窗口启动校验通过，普通配置已恢复。
