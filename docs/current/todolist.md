...
**2026-09-26 B6 设置整页批次已提交，下一批为机器页。** `4842520` 将设置重组为画面与体验、
声音、操作三页，宽屏双列、紧凑窗口单列滚动，底部恢复默认／取消／应用始终可达；主菜单与暂停入口
复用同一设置会话，应用期间锁住草稿与关闭入口，并补齐成功、渲染回退及世界画面重启提示。
Debug／Release 客户端、资源 114、P11A 99 及资源静态 89 均通过，双语资源冻结为 661 键。
没有启动客户端；设置页和上一批世界入口的正常窗口、双语字号尺寸及性能仍待 B6 集中验收。
接下来统一熔炉／破碎机的输入—条件—进度—输出层级和操作反馈，再实现真实世界预览并封板 B6；
完整 Goal 未完成。
