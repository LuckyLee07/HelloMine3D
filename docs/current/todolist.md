...
| 方向 | 已落地与现有证据 | 剩余范围 |
| --- | --- | --- |
| 1 启动与 HUD | 资源排序/打包、稳定状态区、大字号合成布局、前景预警；r4 独立正常菜单/设置/保存重开、r16/r17 有限正常界面自测及诊断布局 | 完整正常容器和持续输入场景仍待验。 |
...
| 5 天空与光照 | 云间留白、方向雾与接触区域复查；r19 云团受光/边界过渡，36 次定向性能通过 | 人类舒适度不声明；最终版本各场景正常体验待整合。 |
| 6 导航与生物 | 三档地图、当前区域、已访问地标；r17 正常切世界刷新；r21 肩髋/头口支点与斜向步态修正、12 次性能通过；r22 面部和材质分区；r23 补足颈部连接，定向验证、性能复核与候选交付完成 | 正常改块/移动/地标生命周期与完整战斗待验；下一批处理动作状态突跳并校正展示夹具比例/时序。独立动物体系尚未实现。 |
| 7 整合与交付 | r11 全矩阵 72 次及掉落物 24 次审计通过；后续按受影响范围逐批性能与候选交付，最近完整候选为 `delivery-r23` | 最终版本正常创建/进入、移动、采集、制作、工具、容器、战斗、设置、保存重开尚未全部验收，Goal 未完成。 |

本批 r23：客户端与 P11E 双配置构建通过，关节各 15/15、P11E 各 35/35、GPU 93/93；
缩短颈部的故障负例被检出。五组静态捕获及 41 个连续时间点，实际审阅七张最终原图。
首次 24 次性能的 Off/静止组失败保留；预先固定的六次反序复核与全部同条件轮次汇总通过，
共 30 次、134,881 帧，汇总 P95/P99 最大比值 1.057365/1.029120。
`delivery-r23` 的 111 项文件、326 份源码和无窗口启动校验通过，普通配置已恢复。

自动诊断保持隐藏/noActivate；普通预览已按用户指令保存退出，历史包和用户存档保留。
macOS 单调时钟修复和睡眠中断样本分类已有证据，历史 Reduced 停摆根因仍未证实；E2 性能问题独立保留。
正常持续输入按用户要求暂缓，不再重复权限排查；姿态混合、脚掌贴地和动物方向没有被上述限定结果计为完成。
