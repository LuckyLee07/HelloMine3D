...
当前 r11 完整 72 次后台对照、12/12 组及独立审计已通过，P95/P99 最大 1.0712×/1.0555×，门槛 1.10；两版共用原生窗口/计时修复与 17 份依赖，实际 1280×720，无截图读回，电源记录未发现睡眠覆盖样本。恢复候选与正常配置后，已构建同计时掉落物基线并开始 24 次专项。专用工作包现由专项 runner 独占且暂用诊断配置，结束时恢复再核对。连续行进观察、正常玩法及最终交付仍待完成，详见上述执行记录的当前恢复入口。
