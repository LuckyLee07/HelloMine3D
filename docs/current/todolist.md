...
| 方向 | 已落地与现有证据 | 剩余范围 |
| --- | --- | --- |
| 1 启动与 HUD | 资源排序/打包、稳定状态区、大字号合成布局、前景预警；r4 独立正常菜单/设置/保存重开、r16/r17 有限正常界面自测及诊断布局，r25 当前包菜单/设置/保存重开自测 | 完整正常容器和持续输入场景仍待验。 |
| 2 E6 与材质 | terrain v12；草/叶/花/土/木/石统一；r13 连续诊断覆盖林下、林缘、草甸、山麓 | 正常连续行进体验待验，诊断相机不等同玩家移动。 |
...
| 5 天空与光照 | 云间留白、方向雾与接触区域复查；r19 云团受光/边界过渡，36 次定向性能通过 | 用户新反馈阴影闪烁，优先定位并修复；人类舒适度不声明。 |
| 6 导航与生物 | 三档地图、当前区域、已访问地标；r17 正常切世界刷新；r21 肩髋/头口支点与斜向步态修正、12 次性能通过；r22 面部和材质分区；r23 补足颈部连接；r24 动作过渡与真实展示比例，24 次定向性能和候选交付通过 | 正常改块/移动/地标生命周期与完整战斗待验。脚掌贴地未验证；独立动物体系尚未实现。 |
| 7 整合与交付 | r11 全矩阵 72 次及掉落物 24 次审计通过；后续按受影响范围逐批性能与候选交付，最近完整候选为 `delivery-r24`；r25 完成 11 组整合静态画面及当前包正常菜单旅程 | 正常创建/进入、设置与有限保存重开已补；移动、采集、制作、工具、容器、完整战斗尚未验收，Goal 未完成。 |

r25 整合/正常自测没有代码变更；8 张正常窗口证据与 11 组静态场景身份通过。
发现近镜头紫色方盒投射物；按用户最新反馈优先处理阴影闪烁，再修整投射物。

上一工程批 r24：客户端与 P11E 双配置构建通过，关节/过渡/夹具各 28/28、P11E 各 35/35；
三项故障负例被检出。八组静态捕获、两段各 51 个不同时间点的 25 fps 序列，实际审阅
11 张最终原图与 3 张对照原图。24 次静止/流送性能及逐帧身份审计通过；
`delivery-r24` 的 111 项文件、327 份源码与无窗口启动校验通过，普通配置已恢复。
最初 50 fps 采集和视频编码失败保留；历史 r23 性能失败/复核仍在执行记录，不被本批覆盖。

自动诊断保持隐藏/noActivate；普通预览已按用户指令保存退出，历史包和用户存档保留。
