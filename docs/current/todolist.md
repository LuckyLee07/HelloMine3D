...
**2026-09-26 B6 机器页批次已提交，下一批为真实世界预览。** `391df78` 将熔炉／破碎机统一为
输入—加工状态与进度—产物层级，保留真实五态、能力接口和原子转移；新增满背包、不兼容、槽位冲突、
并发变化及动力已满反馈，投放／领取遵守角色与 `insertable`／`extractable`，普通界面不暴露内部 tick。
Debug／Release 客户端、资源 114、P11A 99、C1 17、C2 51、C3 68 及资源静态 89 均通过，
双语资源冻结为 681 键。没有启动客户端；世界入口、设置、机器页的正常窗口、双语字号尺寸及性能仍待
B6 集中验收。接下来只为选中世界读取约 3–4 KiB 的真实探索预览缓存，不在菜单加载整份地图，也不
根据 seed 推测地形；完成后集中封板 B6。完整 Goal 未完成。
