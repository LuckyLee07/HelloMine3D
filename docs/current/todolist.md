...
专用 r7 又修复表现时钟大增量/长会话精度：双配置计时各 5/5、物品各 8/8、地图各 10/10 通过；Full/Reduced/Off 共 96 帧后台动作检查通过。随后完成 72 次后台性能对照与独立审计，实际 2560×1440，关闭截图读回；r7 工作包和正常配置均已核对恢复。掉落物四组 128 帧审阅发现薄物品侧转消失，r8 已修复：双配置客户端构建与物品各 13/13 通过，同倍率修复前后 192 帧核对及限定范围视觉检查通过；r8 相关性能待补。历史 Reduced 停摆根因尚未证实，完整正常动作与最终整合交付待验。
