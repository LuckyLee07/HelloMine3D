...
**2026-09-25 Goal 阶段边界已修订：先交付当前目标，再进入完整视觉精修。**
按[修订 Goal](adventure-experience-goal-prompt-2026-09-22.md)第三节，先收口当前地图等缺陷和
必要验收，完成 B6 剩余界面 → B7 地下 → B8 完整玩家／第三人称 → B9 声音 → B10 整合交付。
B1–B5 未验项随相关会话补齐；外部阻塞只暂停依赖部分。新规划不整体追加当前退出条件，
也不用于把原有最低结果后移。宿主 Goal 已于 2026-09-26 随用户继续指令恢复为 `active`，
继续沿用同一 objective 和唯一执行报告，没有新建重复 Goal。

**2026-09-26 B6 世界预览批次已提交，下一工程批进入 B7。** `3d91413` 增加固定 49×25、
8 m／格、最多 4096 B 的真实探索预览缓存，`35579a8` 只为当前选中世界读取并绘制该缓存；未知区域、
玩家朝向和地表材质来自真实探索数据，不遍历加载全部世界，也不根据 seed 推测。旧／坏／异世界／过期／
不安全缓存软回退，预览失败不影响权威保存；备份排除缓存，恢复前安全失效，避免同秒同修订误用旧中心。
Debug／Release 的预览 36、运行时地图 38、世界目录 73、事务 18、资源 114 及客户端构建均通过。
本批没有启动客户端；世界入口、设置、机器页和世界预览的双语三字号两尺寸、正常交互及同机性能仍待一次
B6 集中会话，不拆成逐页冷启动。并行推进不依赖窗口的 B7 地下基线、合同和实现，不删除 B6 退出条件。
完整 Goal 未完成。

**2026-09-25 场景与视觉精修规划已落文，作为后续阶段。** 用户要求将最新视觉分析整理为规划，
见[场景与视觉体验精修规划](visual-experience-polish-plan-2026-09-25.md)：材质、植被、地形过渡、
天空远景、阴影、水域、动物／玩家、地标建筑、地下、整套 UI 共十个方向，明确现状、优先级、
实施范围与验收；原 Goal 达标交付后建议从林地—河岸—岩坡路线开始。它承接既有 B0–B10，
不重置总目标或删减地下／玩家／声音，聚落扩展单列后续候选。本次不启动新的代码、资源、构建或客户端批次；
当前已交付地图及仍未完成的动态／性能检查保持下述状态。

**2026-09-25 地图增量更新修复已交付。** `108cc31` 复用重叠观察、优先补边缘，有界保留
会话内精细地表；重新打开保留视图参数，持久地图格式不变。双配置构建与 HUD 44／地图 74 通过，
`MapIncremental.app` 119 项发行内容匹配。窗口已被用户操作，保留运行与只读原图；普通移动重开、
动态改块及本批三对性能仍 NOT_RUN。详情见唯一执行报告，本阶段不等同完整 Goal 完成。

**2026-09-25 平面地图细节与构图修复。** 根据用户对照设计稿的反馈，`f1ae92d` 将当前区域改为
基础 2 m 的真实地表细层，沿用小地图配色，缩放不再丢弃附近的树冠、岸线和高差；历史底图为
129×129，默认适配当前区域，拾取使用实际显示的细列。双配置构建、地图／输入／归档聚焦通过，
同 seed／XZ 的后台原图已核对，普通输入不冒充通过。新版独立工作包 `MapDetail.app`；
原 `MapTruth.app` 与 `UiRefinement.app` 保持不变。性能与最终身份见
[唯一执行记录](../reports/adventure-experience-execution-2026-09-22.md)末节，完整 Goal 仍未完成。

**2026-09-25 地图详情一致性修复已提交，实机完整对照待验。** `88cf14b` 修复平面图只归档脚边、
立体图复用小地图范围，以及高空玩家箭头偏离地表的问题。详情根据渲染距离有界采样真实驻留区块，
观察同时更新持久地图；未知区块不生成、不加载。双配置构建与聚焦通过，Release 完整世界
2015/2015 PASS，独立 `MapTruth.app` 119 项发行文件核对通过。用户接管该窗口后停止输入，
只读实拍确认 RD8 的 292 m 范围；原问题位置的两视图完整对照、动态编辑界面检查与三对性能
保持 NOT_RUN。MapTruth 与原 UiRefinement 均作为用户客户端保护，截图仅保留本地。
详见[唯一执行记录](../reports/adventure-experience-execution-2026-09-22.md)末节。

**2026-09-25 UI 定稿细节修正已交付，完整 B6 仍 Doing。** 在上轮 `9964e31` 基础上，
六个中文小批次提交至 `1a0b41b`：地图投影适配／像素材质／信息侧栏、任务与暂停构图、轻量 HUD、
文字编辑与 HUD 快捷键隔离，以及小窗口边界。双配置构建、必要聚焦回归、普通菜单交互、
基地标记保存重开及部分双语字号尺寸组合通过。独立 `UiRefinement.app` 为当前修正版；
原 `SkyWorkbench.app` 正在被使用，保持包及实例不变。本轮三对性能因并行用户客户端保持 NOT_RUN，
不能沿用上轮通过结果。截图与设计对照页仅保留本地。完整字号尺寸矩阵、普通采集回路及
世界／机器／设置整套升级仍待完成；详见[唯一执行记录](../reports/adventure-experience-execution-2026-09-22.md)。

**2026-09-22 用户明确启动：冒险世界体验升级（Doing，完整 Goal）。** 按 [迭代规划](adventure-experience-plan-2026-09-22.md) 与 [完整 Goal](adventure-experience-goal-prompt-2026-09-22.md) 持续实现：B0 基线 → B1 天空 → B2 前期任务 → B3 动物 → B4 目的地与建造 → B5 地图 → B6 界面 → B7 地下 → B8 玩家 → B9 声音 → B10 整合交付。全部方向均为必做范围；B1 云层／区域氛围仍待阶段验收，B2a–c 已本地提交而普通路线待验，B3 动物领域及模型工程已本地提交、普通画面／性能待集中验收，B4 地点双布局、区域建造配方、接近线索及三类建筑示例已有工程实现，普通探索、画面与性能仍待集中验收。按用户要求减少重复客户端启动，代码／资源／存档先无窗口验证，必要的连续画面、普通输入和性能集中验收；进度、失败、真实缺项和恢复入口集中在[唯一执行记录](../reports/adventure-experience-execution-2026-09-22.md)。本顺序取代下文历史目标的恢复顺序，已交付地形与 HUD 保留为基础，不把首批完成当成总 Goal 完成。
