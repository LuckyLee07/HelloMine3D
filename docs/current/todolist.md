...
当前 `Doing`。按 HUD/启动 → E6 与材质 → 真实水深/岸线 → 手持/掉落物 → 天空/光照 → 导航/地标/敌人
...
新增顺岸风驱水面漂移已单独提交 `864c86e`：双配置世界各 1190/1190、资源各 112/112、GPU 与窗口冒烟通过；r5 前台性能完成 18/72 次、三组通过后按用户要求停止。r7 已补水岸开启/关闭及水下共 192 帧后台检查，正式后台性能矩阵执行中。
...
专用 r7 又修复表现时钟大增量/长会话精度：双配置计时各 5/5、物品各 8/8、地图各 10/10 通过；Full/Reduced/Off 共 96 帧后台动作检查通过。随后启动 72 次后台性能对照，实际 2560×1440，关闭截图读回；执行期间工作包交替替换两版、使用诊断设置，结束后核对恢复。历史 Reduced 停摆根因尚未证实，完整正常动作与最终整合交付待验。
自动/GPU/固定图不替代完整正常玩法，Goal 仍进行中。
