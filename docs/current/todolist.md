...
r15 修正版将共同介质过渡扩展至水面下 0.05–1.0 m，保持最多两次驻留查询；双配置客户端构建及 V10C 各 28/28 通过。四段 388 帧隐藏采集、与基线共 776 帧核验及 18 张原图审阅完成；白天出水 ROI 最大变化为 21.36/255，低于失败初版但略高于 r14，不作为舒适度门槛。开启大气时远景亮带消除，关闭大气的旧亮带仍列为未解决项。36 次、6 组定向性能及独立审计通过，174,097 帧，P95/P99 最大比值 1.006863 / 1.008316，门槛 1.10；本批验证完成，按中文形式本地提交。
用户明确表示观察结束后，已通过菜单保存并退出预览客户端；普通预览存档和 r12 交付包保留。专用工作包恢复隐藏/noActivate 自动验证，后续自动启动不抢前台。
自动/GPU/相机诊断不替代完整正常玩法，Goal 尚未完成；接续当前候选客户端更新和完整玩法验收。
