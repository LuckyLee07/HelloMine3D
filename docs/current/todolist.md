...
| 2 E6 与材质 | terrain v12 植被及材质统一；r28 terrain v13 沙丘、岩台、湿地和起伏/盆地已实现，工程、限定原图、36 次定向性能和正常新建/保存重开通过 | 正常连续行进体验待验，诊断相机不等同玩家移动。 |
...
| 7 整合与交付 | r11 全矩阵 72 次及掉落物 24 次审计通过；后续按受影响范围逐批性能与候选交付，最近完整候选为 `delivery-r28`；r25 完成 11 组整合静态画面及当前包正常菜单旅程 | 正常创建/进入、设置与有限保存重开已补；移动、采集、制作、工具、容器、完整战斗尚未验收，Goal 未完成。 |
...
自动诊断保持隐藏/noActivate；r28 正常窗口自测后已保存退出，无剩余客户端或性能任务。原七个世界和用户设置未变，另新增一个 v13 自测世界；历史包保留。
