...
2026-09-17 用户授权从产品、用户和美术角度体验并直接实施一轮综合视觉升级：固定正常字重、
HUD/快捷栏/物品卡、真实地表小地图、草地光色与尺度层次。当前 `Doing`，详细范围见
[视觉升级方案](visual-upgrade-plan-2026-09-17.md)，实际验证与恢复入口见
[执行记录](../reports/visual-upgrade-2026-09-17.md)。为避免与同时推进的 E5 水系互相覆盖，
本轮使用冻结 terrain v10 的隔离检出验证，主工作区保留 E5 的既有改动；实机画面和性能不能
由离屏 GPU 或历史 E4 证据代替。
...
`Engineering Done / 自动验证 PASS / 窗口与性能待集中验收`：起始 commit `3e732a1`，实现
提交 `7be7d2c`，小地图远端夹具稳定性修复提交 `cf521b8`。16 个固定正负坐标林区的 144 个
...
[冻结清单](../reports/ecology-e6-v11-vegetation-baseline-2026-09-17.json)。固定 v11/v12 林地画面、
同客户端三轮性能和中文正常玩法仍须在唯一工作 `.app` 中集中采集，不以自动结果代替。
