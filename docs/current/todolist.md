...
当前未完成，Goal 已暂停。按 HUD/启动 → E6 与材质 → 真实水深/岸线 → 手持/掉落物 → 天空/光照 → 导航/地标/敌人
...
专用 r7 又修复表现时钟大增量/长会话精度：双配置计时各 5/5、物品各 8/8、地图各 10/10 通过；Full/Reduced/Off 共 96 帧后台动作检查通过。随后完成 72 次后台性能对照与独立审计，实际 2560×1440，关闭截图读回；r7 工作包和正常配置均已核对恢复。另完成掉落物四组共 128 帧后台采集，整组视觉审阅待完成。历史 Reduced 停摆根因尚未证实，完整正常动作与最终整合交付待验。
自动/GPU/固定图不替代完整正常玩法，Goal 尚未完成，当前暂停。
