...
| 方向 | 已落地与现有证据 | 剩余范围 |
| --- | --- | --- |
| 1 启动与 HUD | 资源排序/打包、稳定状态区、前景预警；r4/r16/r17/r25 有限正常界面与生命周期自测；r34 统一行囊槽位、箱子与合成层级、体积预览和固定操作，双语三字号两尺寸、限定 CUA 存取/制作及相关性能通过 | 下一批重做常驻 HUD；英文窄窗口仍有生命/提示拥挤和区域名截断，机器与菜单整体设计未完成；完整正常容器和持续输入仍待验。 |
...
| 7 整合与交付 | r11 全矩阵 72 次及掉落物 24 次审计通过；后续按受影响范围逐批性能与候选交付，最近候选为 `delivery-r34`；r25 完成 11 组整合静态画面，r32 补当前 v15 正常菜单建档/保存重开，r34 补隔离诊断 CUA 物品存取/制作 | 正常创建/进入、设置与有限保存重开已补；移动、采集、完整正常制作/工具/容器链及战斗尚未验收，Goal 未完成。 |
...
自动诊断保持隐藏/noActivate；r34 所有进程已退出，候选、启动器与正常设置恢复，客户端关闭。
