...
当前版本。地图三档范围引入 runtime settings v10；当前新世界 terrain v14 沿用 v13 地貌并重做三类地标，world save v12 保持。
...
| 7 整合与交付 | r11 全矩阵 72 次及掉落物 24 次审计通过；后续按受影响范围逐批性能与候选交付，最近完整候选为 `delivery-r31`；r25 完成 11 组整合静态画面及当前包正常菜单旅程 | 正常创建/进入、设置与有限保存重开已补；移动、采集、制作、工具、容器、完整战斗尚未验收，Goal 未完成。 |
...
自动诊断保持隐藏/noActivate；r31 性能任务已结束，客户端已关闭，原八个世界和用户设置保持。实际提交及证据入口记录于本地 `goal-recovery-r7.json`；历史包保留。
