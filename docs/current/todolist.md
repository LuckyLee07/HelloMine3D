...
**2026-09-26 B6 世界入口首批已提交，下一批为设置整页。** `08c97e3` 完成最近世界继续、
主菜单设置入口、分层世界列表、真实时间／版本／难度信息及紧凑滚动；活动目录失败会阻止进入，
可恢复区失败只禁用恢复功能，不再锁死有效世界。真实预览数据源尚不存在，本批明确显示缺失回退。
Debug／Release 客户端、资源 114、目录 60、P11A 99 及资源静态 89 均通过。没有启动客户端；
正常窗口与矩阵仍待 B6 集中验收。接下来整理设置页的显示／操作、音频、控制层级和紧凑操作区，
再处理机器页、真实预览与 B6 封板；完整 Goal 未完成。
