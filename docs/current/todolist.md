...
当前 `Doing`，Goal 已恢复。按 HUD/启动 → E6 与材质 → 真实水深/岸线 → 手持/掉落物 → 天空/光照 → 导航/地标/敌人
...
专用包复用 r13 二进制及 r14 shader、视觉采集后正常配置已恢复；r12 交付包保留。水下远端亮边继续单独定位；相机诊断不代替正常行走、互动或最终玩法验收。
自动/GPU/固定图不替代完整正常玩法，Goal 已恢复，尚未完成。
