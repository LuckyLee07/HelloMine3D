...
r12 修复大字号合成网格被挤出视区：配方/材料与网格分列，底部按钮固定，双配置构建及十组中英文/字号/窗口布局原图检查通过。专用工作包与正常配置已恢复，目前没有采样 runner；性能证据保留 r11 身份，r12 仅修改打开的合成面板布局。连续行进观察、正常玩法及最终交付仍待完成，详见上述执行记录的当前恢复入口。
