...
- terrain v5 的纯 `TerrainFoundation::sample` 以 floor lattice 和 uint64 hash 覆盖 signed world
  ...
  v19 世界中，洞口规划额外返回纯几何入口清单，植被在通道投影周围留空，
  ...
  v21 仅在地点建筑前方按实际林地、水岸或高地列铺设短程地面线索，
...
不新增世界状态、纹理或渲染目标。B1e 将有界云层抬至 308–332 m，以三层密度组合形成轻薄云群，
覆盖与一次向光密度查询分离，避免硬面及实体法线；整体随风连续漂移，清空／薄缘／浓密层次并存。
...
`RegionalAtmosphere` 在进入水介质前按当前生成器的自然 biome／高度派生区域空气：
64 m 世界网格四角缓存，跨单元最多八次纯查询，单元内只插值；固定 88 字节，切世界清空。
它协调雾、地平线、天顶和云的轻微区域色差，保留世界时间、日光、云运动与水面权威值。
增强大气关闭时跳过，完整入水时保持原水下雾；不加载区块或写入存档，详见
[冒险天空合同](../contracts/adventure-sky-contract-v1.md)。
...
常驻 HUD 采用共享细边框半透明面板、独立五格快捷槽与连续细生命条；叶片／心形沿用 Ogre 所持
纹理的 Alpha／nearest 采样，文字、计数和进度即时绘制。完整指导保留在任务日志及指针悬停，
任务两行摘要、按键提示和紧凑窗口两侧通知仅派生布局；
...
未增加保存字段、地图缓存或渲染后台。
2026-09-25 细节修正：首次地图入口默认局部立体，本世界内保留视图选择；立体图按真实几何边界适配，
背景网格与地表共用投影，材料纹理复用正式图集并按顶面／侧面选择。平面图将显示缩放与
4–64 m 查询间距分开，首次按当前窗口内已有记录适配；单次查询仍不超过 65×65 格。
未知区、标记位置与存储精度不变，关闭与切世界不产生持久视图字段。
...
归档的 129×129 个显示格，支持 4／8／16／32／64 m 每格、离开区域平移复看；缩小范围
只汇总视口覆盖的已记录 4 m 单元，每格保留代表样本的真实坐标供标记使用。静止时
每秒刷新一次值快照，不请求远区生成。局部立体图只使用当前驻留样本，平面图使用同次真实观察更新后的历史归档；
卸载清除实时缓存而不擦除历史。平面另外维护最多 257×257 的 2–16 m 真实驻留列，
使用小地图材料／高差配色覆盖较粗历史底图，缩放不丢弃附近采样；拾取与标记使用实际显示的细列。
`MapSurfaceRegion` 按世界坐标复用重叠观察，新边缘优先、其后继续轮询以刷新编辑与卸载。
`SurfaceMapHistory` 将小地图及平面的已有 2 m 观察保留为最多 2048 个 16 m 页；
当前世界会话内按最近观察淘汰、视口裁剪，未知不擦历史，切世界清空；不新增磁盘状态。
三视图互斥消费既有 195 列／30 Hz 预算，平面新增临时 CPU 数据预算小于 6 MiB
（不包含 ImGui 绘图缓冲），最多 131072 个细历史格；持久精度、格式和容量不变。
首次／回到玩家按当前渲染范围适配至短边 82%，不按未补齐的采样行改变缩放；手动缩放／平移后保持用户视图，关闭再打开也不重置立体方向和视图参数。
...
创建／移动只接受已观察的 4 m 单元，必要时最多查询一个驻留地表列，不同步生成远区。
Ogre 地图页提供个人标记管理和已探索格选择，确认删除后调用 World 命令；平面图绘制
基地／普通／追踪标记，小地图从当前世界读取追踪点并给出北向方位和水平距离。
UI 状态切世界清理，只有 World 标记集合进入地图文件。实际窗口交互和布局待验。
`ExplorationNavigation` 对已知坐标计算北向八方位和水平距离，先扩宽整数避免
极端旧世界坐标差溢出；平面图仅将真实放置／交互记录的界石坐标与对应任务阶段关联，
不查询生成器。World 在放置／拆除事件和实际界石使用时更新独立地图文件的已知坐标，
同一事务与备份路径保留；旧文件无该字段时不预测补齐。
约束见[探索地图合同](../contracts/adventure-exploration-map-contract-v1.md)。

2026-09-24 地图审查修复：`exploration.hmap` v4 将最近发现的界石与任务绑定分成两项有界
