...
| `difficulty_profile_version`, `difficulty_id` | Versioned world-difficulty identity. World-entry, streaming, scaled-gameplay and playable runs are comparable only when both values match. |
...
script and the same code (see `docs/reports/chunk-streaming-regression.md` R4), and it
...
contract and pass/fail fixtures are tracked in `docs/current/validation-matrix.md`,
`docs/current/runtime-validation.md` and the archived project ledger. Later milestones must not copy these thresholds
...
`docs/contracts/operation-performance-timing-contract-v1.md`.
...
Pair this with `docs/current/render-regression-smoke.md`: performance baseline checks
