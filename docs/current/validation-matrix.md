...
| 持久探索地图 | B5a／B5b 运行 `bash scripts/verify_exploration_atlas.sh` 双配置，覆盖未知、已知空列、坐标极值、刷新、满额保持历史、文件身份／校验、事务故障和损坏回退；B5c 同脚本检查 UTF-8、稳定 ID、单一基地／追踪、修改删除和容量，以及文件 v1→v2 读取、标记往返和篡改负例。备份／恢复运行 `HelloMine3DWorldBackupSmoke`，世界运行用 `EXPLORATION_MAP` 双配置检查真实采样、远区拒绝、World 标记命令、保存重开、损坏隔离，并编译双配置客户端、运行 Release 完整世界回归。平面图首批检查双配置客户端与资源包、`check_assets.sh`，不凭无窗口编译宣称界面可用；集中窗口核查平面／立体切换、标记、目的地真值、双语与两窗口、正常返回路线及同机三对性能。存储接线通过不代表 B5 完成，见[探索地图合同](../contracts/adventure-exploration-map-contract-v1.md)。 |
