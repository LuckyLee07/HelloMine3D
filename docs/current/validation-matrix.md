...
| 持久探索地图 | B5a 纯数据层运行 `bash scripts/verify_exploration_atlas.sh` 双配置聚焦未知、已知空列、负坐标、极值、编辑刷新、满额保持历史和清空。接入真实世界与文件后补地图格式／事务／备份故障、旧世界默认、损坏回退、双配置构建及完整世界回归；界面阶段核查平面／立体切换、标记、目的地真值、双语与两窗口、正常返回路线及同机三对性能。数据层通过不代表 B5 完成，见[探索地图合同](../contracts/adventure-exploration-map-contract-v1.md)。 |
