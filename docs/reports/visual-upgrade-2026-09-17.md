...
- 专用工作路径 `round2-compact-layout-recheck-r3/Diagnostic.app` 已刷新到上述 r6，bundle id 仍为 `local.hellomine3d.visual-layout-check`；隐藏采集结束、正常配置已恢复，A/B 存档保留。321 个第一方源码和三个修改后的原生窗口源码与构建检出一致。
- 前台 runner 已停止，无运行 handle。下一步在该专用包后台调查 Reduced 静止及新水面动态，再准备相同原生窗口修复的视觉基线进行后台性能对照。r2 为 1280×720 实际像素，r5 前台为 2560×1440；两者均不得改标为当前后台结果。
