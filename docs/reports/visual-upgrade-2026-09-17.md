...
- 前台 runner 已停止。水面后台连续帧和精确恢复的后台视觉基线已就绪；先完成新旧版本无截图性能采集预检，再运行完整后台三轮静态/流送矩阵。实际活跃 handle 见本地 `goal-recovery-r7.json` 及后继记录，恢复时重新核对。历史 Reduced 停摆仍未决。r2 为 1280×720 实际像素，r5 前台为 2560×1440；两者均不得改标为当前后台结果。
