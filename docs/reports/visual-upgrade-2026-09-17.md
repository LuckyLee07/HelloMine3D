...
- 专用工作路径 `round2-compact-layout-recheck-r3/Diagnostic.app` 已在正常退出后刷新到 r5，bundle id 仍为 `local.hellomine3d.visual-layout-check`；窗口冒烟结束、正常配置已恢复，A/B 存档保留。321 个第一方源码与主工作区一致，第三方库未改写。
- 下一步在该专用包补新水面动态观察及同条件三轮静态/流送性能。r2 的 72 次性能是旧 shader、1280×720 实际像素；当前 Retina 为 2560×1440，不能混用像素条件或将旧结果标成 r5。
- 正常菜单、设置、保存/重开现已通过，持续移动/采集等输入仍缺可用路径；恢复文档化持续输入后补工具、容器、地标和 Full/Reduced/Off 动作。首次 Reduced 静止现象尚未解释，不以重采掩盖。Goal 仍进行中。
