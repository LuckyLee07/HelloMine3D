...
状态：**综合视觉 Goal 进行中；阶段 1–6 的主要代码与诊断已分批提交；性能 r2 的 12/12 组通过，新水面批次尚待性能复测，正常输入尚未收口**。
...
- 专用工作路径 `round2-compact-layout-recheck-r3/Diagnostic.app` 已刷新到上述 r7，bundle id 仍为 `local.hellomine3d.visual-layout-check`；隐藏采集结束、正常配置已恢复，A/B 存档保留。323 个第一方源码和三个修改后的原生窗口源码与构建检出一致。
- 前台 runner 已停止，无运行 handle。下一步补新水面后台动态，准备相同原生窗口修复的视觉基线进行后台性能对照；历史 Reduced 停摆保留未决，定向修复与正常玩法分别跟踪。r2 为 1280×720 实际像素，r5 前台为 2560×1440；两者均不得改标为当前后台结果。
- 正常菜单、设置、保存/重开已有 r4 证据，持续移动/采集等输入仍缺可用路径；按用户约定不启动前台/CUA，继续不依赖它的工作。首次 Reduced 静止现象尚未解释，不以重采掩盖。Goal 仍进行中。
