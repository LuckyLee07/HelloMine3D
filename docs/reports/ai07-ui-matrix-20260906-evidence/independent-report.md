...
A screenshot with actual start/end UTC and original bytes/CRC is retained in persistent `uiCaptures[0]` under name `creation-approval-blocked`; it has not yet been exported. Original game/editor handles and prior recovery screenshots are retained without reset.
