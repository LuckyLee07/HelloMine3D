...
| R8 | macOS 原生构建与启动未验证 | 跨平台目标受阻 | Xcode 工作区已在 Windows 侧通过 133 项生成合同检查；全局 `WIN32` 宏、异平台头文件路径和 OIS 示例源码均已清理。最终只需 macOS 主机运行 `scripts/verify_xcode.sh` 完成双配置 `xcodebuild`、测试和客户端启动。 |
