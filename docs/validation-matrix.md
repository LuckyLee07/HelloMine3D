...
| `P11-2`（Planned） | terrain v4 身份与固定 seed；山地高度域/陡坡/洞口确定性；旧 v1-v3 世界不回填；新旧边界与结构重复；快速流送/规模玩法新身份 Q1。水文生成不进入本批。 | 在多个固定/随机种子实际寻找山地轮廓和可发现洞口；记录发现时间、可通行性、远景/阴影效果和新旧世界边界异常。 |
| `P11E`（Queued） | 动画状态由玩法状态驱动；判定与渲染解耦；部件/实例/音频上限；身份化掉落的材料追加、经济守恒、目标链和保存恢复；相关性能。 | 不看名称辨识主要敌人、预警与危险窗口；比较各敌人掉落价值；完整走通 Waystone 独特阶段，记录误判、死亡原因和策略变化。 |
