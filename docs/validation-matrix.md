...
高反馈的自动验证；真实硬件记录、其余正式性能预算、长稳运行和干净发行包可以在主要
内容与 UI 稳定后集中完成。Alpha 检查点已有一份 Windows 隐藏场景预算。
...
| Alpha 检查点 | 跟踪 v3 迁移 fixture；Debug/Release 世界与崩溃 smoke；双模式隐藏客户端；Alpha 基线/复测；匹配及错误 PDB。 | R3、H3、Q3、任意 dump 完整栈和其余六个 Q1 场景预算。 |
