...
| 下一产品阶段已选定 | 第 8 阶段以“更像完整游戏”为主线。K4、G2-G6、Alpha 开发检查点以及 N1-N4 均已完成；当前进入 N5 生态与探索。R3 与正式发行证据后置到主要 UI 稳定后集中执行。R4 已由独立的原生 TSan 门禁关闭。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 12：生态与探索（当前批次） | `N5` | 少量有用途的生态差异、资源与 seed 稳定轻量地标。 | 旧世界生成身份不变；加载顺序确定；快速移动和内容规模预算通过。 | N4 已关闭，可以开始。 |
| 13：产品体验 | `N6` | UI 层级、配方可读性、键位与辅助选项、正式音效和低密度音乐。 | UI/输入稳定，R3 真人记录通过，失败提示可观察。 | N5 内容和主要界面结构稳定。 |
...
| M6 可持续游玩与可靠发布 | 已启动，玩法闭环优先 | K1-K4、G1-G6、N1-N4、H1 已完成，当前继续 N5-N6，再集中取得 R3、H2-H3、Q1-Q3 和发行证据。 |
