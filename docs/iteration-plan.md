...
| 下一产品阶段已选定 | 第 8 阶段、Stage 9/N7-N12、BETA-RC 与 Stage 10/VISUAL-RC Windows 工程均已完成；产品方向评审选择 Stage 11 可玩性与操作体验，P11A 是下一开发批次。人工输入按 P11A 的 Physical Input v2 实际覆盖更新，正式产品体验和 macOS 原生证据继续保持 Verify；多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 第 11 阶段 | P11-0 Planned；其余八批 Queued | 可玩性与操作体验路线不计入当前 77 项总账；先补可制造光源，再关闭输入动作冲突和 Physical Input v2，随后按真实试玩证据推进反馈、建造材料、前 30 分钟、探索奖励、地形轮廓、敌人表现和 PLAYABILITY-RC。 |
...
| 1：世界入口（Done） | `K4` | 世界管理服务、列表模型、最近游玩、创建/打开/重命名、备份选择、恢复和可恢复删除，再接入主菜单。 | 44 项 K1/K4 断言、Debug 主程序编译、隐藏主菜单和直进世界冒烟通过；当时后置的真人输入现归未来 Physical Input v2。 | 玩家路径和失败边界已落地；id/目录稳定，显示名可变，删除可恢复。 |
...
主菜单，带测试存档目录的自动化仍可直进世界。正式物理输入记录现归未来 Physical Input v2。
...
`tool-progression-contract-v1.md`；正式持续按键与暂停焦点归未来 Physical Input v2，HUD 观感归独立产品体验合同。
...
应用语义固定在 `runtime-settings-contract-v1.md`；正式物理焦点归未来 Physical Input v2，观感记录归独立产品体验合同。
...
`audio-feedback-contract-v1.md`；R3 v1 只保留历史十二项输入基线，当前缺口归未来 Physical Input v2，人工听感改由独立产品体验合同记录。
...
前 30 分钟的选择、探索奖励、地形轮廓和战斗表现。当前只允许 P11-0 开始；其余批次必须等
前一批真实试玩退出后逐个开放。权威诊断、数据版本预案、Physical Input v2、PLAYABILITY-RC
和非目标见 `docs/playability-experience-roadmap.md`。

| 顺序 | 批次 | 规模 | 核心结果 | 关键护栏 |
| ---- | ---- | ---- | -------- | -------- |
| 1 | P11-0 世界光源（Planned） | M | 可制造火把、Rose 修正、metadata 熔炉发射和真正翻转时的局部重光照。 | BlockId/Material 末尾追加；旧 v11 活跃熔炉规范化；暂停保持亮度、卸载不改状态；不升持久化版本。 |
| 2 | P11A 核心操作手感（Queued） | M/L | 分离使用/放置/格挡，补鼠标重绑定和冲刺/潜行模式，统一灵敏度与焦点/捕获恢复。 | settings 预计升 v7；旧版确定迁移；目标 Windows 真实执行 Physical Input v2，自动事件不能冒充真人输入。 |
| 3 | P11B 动作反馈（Queued） | M | 手/工具、裂纹、粒子、命中/受击/冷却、拾取和音频微变体。 | 反馈不改变玩法判定；不混入敌人掉落经济；镜头类效果可调/可关；低配路径保持可读。 |
| 4 | P11-1 建造材料与工具（Queued） | M/L | 木板、圆石、门/活板门和 axe/shovel 的最小建造与工具选择。 | 枚举末尾追加、旧存档零迁移；配方/掉落守恒；工具差异可感知但不新增 tier。 |
| 5 | P11C 前 30 分钟（Queued） | M/L | 建造、成长、探索并行循环，至少两个可见选择与自然配方发现。 | 用新世界完整 30 分钟试玩记录判断；首版不强推饥饿。 |
| 6 | P11D 探索奖励（Queued） | L | 结构专属资源/蓝图、实用能力和生态差异经济。 | 奖励必须改变能力或下一步计划；新增持久状态当批冻结 save/terrain 迁移。 |
| 7 | P11-2 地形轮廓与洞口（Queued） | L | 山地高度域、陡坡和可发现天然洞口，形成 terrain v4。 | 旧世界不回填；新旧边界与相关 Q1；湖泊/河流/瀑布不进入本批。 |
...
定向自动回归和 10-30 分钟真实试玩；P11C 至少覆盖完整前 30 分钟。P11F 只集中复核已有证据，
不能把全部真人体验拖到阶段末尾。
...
| 物理输入验收 | `docs/manual-input-acceptance-v1.md` 只保留十二项历史基线；输入、窗口系统、容器 UI 或战斗交互变更按未来 Physical Input v2 人工记录。 |
...
| M9 可玩性与操作体验 | P11-0 Planned | P11-0 先补可制造光源；P11A 随后关闭输入动作冲突、鼠标/模式设置和 Physical Input v2；其余七批只有在前序真实试玩通过后才逐批开放。 |
