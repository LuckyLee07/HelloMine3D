...
单机旅程。`FS1-N12C` 和 BETA-RC 工程封板均已完成；当前没有后续开发批次，只有延期的
R3 真人验收仍保留为 `Verify`。
...
| 16 | BETA-RC（工程 Done） | 已重建迁移、性能、长稳、崩溃、自动输入预检和发行包基线。 | 六类 Q1 与两档 1800 秒 Q3 通过；bundle 已验证；R3 延期，故不建本地标签、不 push。 |
...
| M7 Beta 单机游戏闭环 | 工程完成，待 R3 | 首次进入、视觉/HUD、胜利、战斗、探索结构、资源经济、难度/胜利后事件、双语、本地化音频与 BETA-RC 工程证据均已关闭；RC0/R3 剩余人工项继续延期。 |
