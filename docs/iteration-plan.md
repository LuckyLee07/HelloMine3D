...
| 风险 | 说明 |
| ---- | ---- |
| 目标 Windows 更新门禁待复跑 | macOS 已验证版本化的 27 项 Xcode 工程图、Debug/Release 构建、测试和真实窗口启动；Windows 上次证据早于 G1/K1 与当前工程图合同，需在目标机复跑更新后的 PowerShell 门禁。 |
...
| 第 8 阶段 | 已启动，仍待 R3 封板 | `K1` 世界目录合同和 `G1` 配方注册表已完成，`Q1` 已固定六类性能契约；`K2-K4`、`H1-H3`、`Q2-Q3`、`G2-G6` 仍按依赖推进 |
...
1. 仍需由真人在目标 Release 构建上完成 `R3`，同时关闭 `D2`、`D4` 和 `D6`；
   不依赖输入的 K1/Q1 合同与 G1 基础数据层已先行完成，但这不替代阶段封板。
...
| 2：数据安全 | `K2-K3`、`Q2` | 完成事务保存、备份恢复和保存/启动耗时采集；故障注入证明旧存档不被破坏。 |
...
| macOS Xcode project | `bash scripts/verify_xcode.sh` 先验证 9 个合同/工程图夹具及实际 27 项工程清单，再构建 Debug/Release、执行九项测试和客户端启动探针；陈旧/缺失工程、重复或多分组引用、手工目标顺序及第一方编译告警均会使门禁失败。 |
...
| 存档故障与恢复（第 8 阶段） | K2/K3 交付故障注入和恢复 harness；验证失败不会覆盖最后有效存档，备份容量有界且恢复完整。 |
...
| M6 可持续游玩与可靠发布 | 已启动，待 R3 与后续任务 | K1 与 G1 已完成，Q1 的可移植合同已落地；继续交付 K2-K4、H1-H3、Q2-Q3、G2-G6 及目标 Windows/真人证据。 |
