...
| 风险 | 说明 |
| ---- | ---- |
| 目标 Windows 更新门禁待复跑 | macOS 已验证版本化的 29 项 Xcode 工程图、Debug/Release 构建、测试和真实窗口启动；Windows 上次证据早于 G1/K1-K3 与当前工程图合同，需在目标机复跑更新后的 PowerShell 门禁。 |
...
| 第 8 阶段 | 已启动，仍待 R3 封板 | `K1-K3` 世界目录/事务保存/备份恢复合同、`G1` 配方注册表和 `Q2` 可移植计时采集已完成，`Q1` 已固定六类性能契约；`K4`、`H1-H3`、`Q1/Q2` 目标 Windows 预算证据、`Q3`、`G2-G6` 仍按依赖推进 |
...
| M6 可持续游玩与可靠发布 | 已启动，待 R3 与后续任务 | K1-K3、Q2 可移植采集与 G1 已完成，Q1 的可移植合同已落地；继续交付 K4、H1-H3、Q1/Q2 目标 Windows 预算证据、Q3、G2-G6 及真人证据。 |
