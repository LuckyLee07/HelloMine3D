...
| 第 9 阶段 | Doing：FS1-N12A 已完成 | `N7-N12` 与 `BETA-RC` 不计入当前 77 项总账；当前从 N12B 正式采样音效继续，范围和回归触发器见 `docs/beta-gameplay-roadmap.md`。 |
...
单机旅程。`FS1-N12A` 的既定批次已完成，当前从 N12B 继续；后续批次只有实际启动时才进入
`docs/todolist.md`。
...
| M7 Beta 单机游戏闭环 | Doing：FS1-N12A 已完成 | 已关闭首次进入、视觉/HUD、胜利、战斗、探索结构、资源经济、难度/胜利后事件和双语本地化；RC0/R3 剩余人工项延后且不阻塞，当前从 N12B 继续，最终由 BETA-RC 集中封板。 |
