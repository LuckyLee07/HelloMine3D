...
| 风险 | 说明 |
| ---- | ---- |
| 目标 Windows 更新门禁被构建问题阻断 | macOS 已验证版本化的 31 项 Xcode 工程图、Debug/Release 构建、十三个测试和真实窗口启动；当前 Windows 前置检查与工程生成通过，但 `WorldRuntimeSmoke`/`Soak` 因未继承 `dbghelp.lib` 在 Debug 链接失败。先修复主干编译，正式 PowerShell 门禁和受控 minidump 证据可以随阶段封板补齐。 |
| 输入仍有平台型证据缺口 | OIS 到控制器的数据边界已自动验证，但真实 Windows 键鼠仍需人工验收；原生 arm64 Apple Clang ThreadSanitizer 已关闭后台加载器的 R4 证据缺口。只剩 `R3`。 |
| 下一产品阶段已选定 | 第 8 阶段以“更像完整游戏”为主线。K4、G2-G6 是开发主线；崩溃诊断和性能预算作为跟随护栏，R3 与正式发行证据后置到综合封板。R4 已由独立的原生 TSan 门禁关闭。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 第 8 阶段 | 已启动，玩法闭环优先 | `K1-K3` 世界目录/事务保存/备份恢复合同、`G1` 配方注册表和 `Q2` 可移植计时采集已完成，`Q1` 已固定六类性能契约；当前先推进 `K4`、`G2-G6`，`R3`、`H1-H3`、`Q1-Q3` 的正式证据在综合封板阶段集中补齐。 |
...
| ---- | ------ | -------- | -------- | -------- |
| 0：恢复基线 | `BLD-1` | 给实际编译 `WindowsCrashDiagnostics.cpp` 的 Windows 游戏逻辑目标补齐 DbgHelp 链接，重新生成工程并恢复 Debug 主干。 | 重跑 H1 便携探针；采集一份不批准阈值的 Q1/Q2 Release 参考快照。 | WorldRuntimeSmoke/Soak 正确链接，Windows Debug 全量编译和定向目标通过。 |
| 1：世界入口 | `K4` | 世界管理服务、列表模型、最近游玩、创建/打开/重命名、备份选择、恢复和可恢复删除，再接入主菜单。 | 沿用 K1-K3 的目录、事务和恢复测试；每个写操作记录阶段计时。 | 玩家可从空目录进入世界并安全返回菜单；id/目录稳定，显示名可变，删除可恢复。 |
| 2：制作闭环 | `G2` | 制作会话、纯预览、最大可制作数量、原子提交、玩家制作区、工作台 3x3 和制作 UI。 | 配方解析、库存容量、失败原子性、关闭/重开/重载守恒。 | G1 数据通过正常玩家路径产生产物；连点、满背包和中断均不复制或吞物品。 |
| 3：成长闭环 | `G3` | 工具定义表、破坏进度、采集等级、速度倍率、掉落限制、耐久和工具状态持久化。 | 先锁定物品实例与存档版本，再改交互；加入固定动作表的性能采样。 | 手、木、石至少形成三级采集能力；旧存档可迁移，升级工具能解锁新资源。 |
| 4：产品外壳 | `G4` | 暂停状态机、设置草稿与快照、应用/取消/恢复默认值、版本化用户设置。 | 自动焦点隔离；区分“立即生效”“重建渲染资源”“下次启动生效”。 | 暂停冻结本地模拟；视距、FOV、灵敏度、反转 Y、音量等设置可预测地保存和恢复。 |
| 5：反馈层 | `G5` | 业务音频事件、2D/3D/listener、分类音量、pause/mute、真实轻量后端和 dummy 后端。 | 资产清单、事件去重、无设备/缺文件降级和基础并发上限。 | UI、方块、拾取、制作、战斗和环境动作有一致反馈，失败动作不误报声音。 |
| 6：可玩 Alpha | `G6` | 整合主菜单、世界、制作、工具、战斗、暂停、音频、保存和继续游戏；补最低限度目标提示与平衡。 | 全流程 fixture、截图、开发者交互冒烟和可比较性能采样。 | 从干净启动无需 debug 注入即可完成完整单机流程，并在重启后继续。 |
| 7：质量封板 | `R3`、`H1-H3`、`Q1-Q3` | 真人输入记录、受控 dump、脱敏 sidecar、离线符号化、下次启动提示、正式预算、压力/soak 和干净包。 | 只修缺陷和预算回归，不在此批新增玩法系统。 | D2/D4/D6/R3 关闭，崩溃和性能合同有目标机器证据，发行包不含隐私数据和符号。 |
...
2. G6 冻结后先完成 R3，并同步关闭 D2/D4/D6；此时 UI、输入和动作语义已经稳定，记录
   不会因后续界面重做失效。
3. 然后关闭 H1-H2，验证 dump、脱敏 sidecar 和离线符号；H3 最后接入已经稳定的主菜单
   和干净发行包流程。
4. Q1 批准目标 Windows Release 预算，Q2 用正常/回归样本证明门禁，再由 Q3 执行快速
   移动、内容规模、内存/队列和长稳场景。正式预算不得靠放宽旧阈值关闭失败。
5. 最后运行全平台构建、存档故障、资源包、截图、真人输入、崩溃、性能、soak 和干净包
   门禁；封板批只修缺陷，不继续增加内容。
...
G6 代表“完整游戏骨架可玩”，并不等于内容完成。只有第 8 阶段封板后，才把下面候选
转成带编号的正式任务，避免当前主线同时扩张。

| 候选方向 | 目标 | 进入条件 |
| -------- | ---- | -------- |
| 成长纵深 | 熔炉/冶炼、铁级工具、食物或恢复资源，形成第二段资源曲线。 | G2/G3 状态模型和存档版本已稳定，数据守恒测试可复用。 |
| 游戏目标 | 轻量任务/成就、阶段目标和一次可达成的会话终点，让玩家不只是在沙盒里自发行动。 | G6 流程证明真实的新玩家阻塞点，先做数据模型再做 UI。 |
| 内容广度 | 增加少量有用途的方块、掉落、敌人和生态差异，而不是只增加装饰资产。 | Q3 内容规模预算获批，每项内容能进入成长或战斗循环。 |
| 体验完善 | 键位重映射、辅助选项、UI 视觉统一、环境音和音乐、首轮引导。 | G4/G5 接口稳定，R3 暴露的真实输入问题已修复。 |
| 发布准备 | 存档迁移演练、版本号/变更日志、安装与卸载、崩溃文件管理和兼容性矩阵。 | H/Q 正式关闭并产生至少一个可重复的发行候选流程。 |
...
| M6 可持续游玩与可靠发布 | 已启动，玩法闭环优先 | K1-K3、Q2 可移植采集与 G1 已完成，Q1 合同及 H1 实现/便携门禁已落地；先交付 K4、G2-G6，再集中取得 R3、H1-H3、Q1-Q3 和发行证据。 |
