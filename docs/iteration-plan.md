...
| 下一产品阶段已选定 | 第 8 阶段以“更像完整游戏”为主线。K4 世界入口、G2 制作闭环、G3 工具成长和 G4 暂停/设置已完成，G5 已进入在制状态，随后推进 G6；Alpha 之后按目标引导、熔炉/铁级、食物恢复、战斗、生态和体验预排内容批次，R3 与正式发行证据后置到主要 UI 稳定后集中执行。R4 已由独立的原生 TSan 门禁关闭。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 5：反馈层（Doing） | `G5` | 业务音频事件、2D/3D/listener、分类音量、pause/mute、真实轻量后端和 dummy 后端。 | 资产清单、事件去重、无设备/缺文件降级和基础并发上限。 | UI、方块、拾取、制作、战斗和环境动作有一致反馈，失败动作不误报声音。 |
