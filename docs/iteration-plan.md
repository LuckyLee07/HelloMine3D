...
| 下一产品阶段已选定 | 第 8 阶段、Stage 9/N7-N12 与 BETA-RC 工程均已完成；人工输入和产品体验继续延期。Stage 10 的 V10A/V10B1 已完成 Windows 工程范围，V10B2 工程实现已完成并等待项目所有者前台视觉关闭，再按 V10B3、V10C-V10E 推进，详细范围见 `docs/visual-quality-roadmap.md`。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 第 10 阶段 | V10B2 前台视觉 Verify | `V10A`、`V10B1-B3`、`V10C-V10E` 与 `VISUAL-RC` 不计入当前 77 项总账；V10A 已按所有者批准的性能例外关闭，V10B1 Windows 工程与画面证据已关闭、macOS shader 子状态保持 Verify；V10B2 自动/隐藏画面证据已完成，等待项目所有者自行前台观察。 |
...
| 顺序 | 批次 | 规模 | 核心结果 | 关键护栏 |
| ---- | ---- | ---- | -------- | -------- |
| 1 | V10A 顶点平滑光照与 AO（Done） | M/L | 四角邻域光照与接触遮蔽替代整面单亮度；共面 byte-identical solid 顶点通过索引复用。 | 保持 32 字节顶点布局；所有者已批准 exact AO 性能例外，最终身份仍在 VISUAL-RC 重跑 Q1/Q3。 |
| 2 | V10B1 材质与图集管线（Done；macOS Verify） | M | 参数化图集/tile/颜色管线。 | 旧图集静态前景像素兼容、shader/资源启动负例和 Windows 双配置通过；macOS Release 窗口待补。 |
| 3 | V10B2 原创材质资产（Implemented，前台视觉 Verify） | L | 37 个原创分面材质与世界/HUD/手持一致性已实现。 | 106 项资产合同、来源/许可、67 项 manifest、资源包、Q1、固定图和干净包通过；待项目所有者自行前台观察 5-10 分钟。 |
| 4 | V10B3 生态着色与确定性变体（Planned） | M | 受控 tint 和坐标稳定的自然 tile 变体。 | 不升级 terrain；变体进入 greedy key；加载顺序、Q1 与短 Q3。 |
| 5 | V10C 定向大气与立体云（Planned） | M/L | 观察方向相关雾色、独立云层视差和云底明暗。 | 关闭时回退 FS2；不引入天气/体积光/碰撞；macOS Release 窗口冒烟。 |
| 6 | V10D 可选方向阴影（Planned） | L | 单近景太阳 shadow map 与 Off/Medium/High 档位。 | settings v5、v0-v4 迁移、双语 key、图形性能身份和能力不足回退。 |
...
实现、验收和本地提交；V10D/V10E 可以默认关闭，但不得跳过 V10A、V10B1/B2 或 V10C 的
基础画面问题。VISUAL-RC 后设置产品方向评审门，再决定进入 `1.0-RC` 或选择一个有试玩证据的
玩法小批次；饥饿、更多内容和成就/统计默认不排期。
...
| M8 视觉质量升级 | V10B2 前台视觉 Verify | V10A 顶点光照/AO 与 V10B1 材质参数管线已完成 Windows 工程范围；V10B2 原创分面材质的工程与隐藏画面证据已完成，待项目所有者自行前台观察后，再按生态变体、大气云层、可选阴影、轻量后处理和 VISUAL-RC 顺序关闭。 |
