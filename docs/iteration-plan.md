...
单机旅程。`FS1-N11A` 的既定批次已完成，当前从 N11B 继续；后续批次只有实际启动时才进入
`docs/todolist.md`。
...
| M7 Beta 单机游戏闭环 | Doing：FS1-N11A 已完成 | 已关闭首次进入、视觉/HUD、胜利、战斗、探索结构、资源经济和难度档案；RC0/R3 剩余人工项延后且不阻塞，当前从 N11B 继续，最终由 BETA-RC 集中封板。 |
