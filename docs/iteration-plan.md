...
| 第 11 阶段 | P11A Planned；P11B-P11F Queued | 可玩性与操作体验路线不计入当前 77 项总账；先关闭输入动作冲突和 Physical Input v2，再按真实试玩证据推进反馈、前 30 分钟、探索奖励、敌人表现和 PLAYABILITY-RC。 |
...
第 11 阶段先解决操作可预测性和动作可读性，再重构前 30 分钟的选择、探索奖励和战斗表现。
当前只允许 P11A 开始；其余批次必须等前一批真实试玩退出后逐个开放。权威诊断、数据版本预案、
Physical Input v2、PLAYABILITY-RC 和非目标见 `docs/playability-experience-roadmap.md`。

| 顺序 | 批次 | 规模 | 核心结果 | 关键护栏 |
| ---- | ---- | ---- | -------- | -------- |
| 1 | P11A 核心操作手感（Planned） | M/L | 分离使用/放置/格挡，补鼠标重绑定和冲刺/潜行模式，统一灵敏度与焦点/捕获恢复。 | settings 预计升 v7；旧版确定迁移；目标 Windows 真实执行 Physical Input v2，自动事件不能冒充真人输入。 |
| 2 | P11B 动作反馈（Queued） | M | 手/工具、裂纹、粒子、命中/受击/冷却、拾取和音频微变体。 | 反馈不改变玩法判定；镜头类效果可调/可关；低配路径保持可读。 |
| 3 | P11C 前 30 分钟（Queued） | M/L | 建造、成长、探索并行循环，至少两个可见选择与自然配方发现。 | 用新世界完整 30 分钟试玩记录判断；首版不强推饥饿。 |
| 4 | P11D 探索奖励（Queued） | L | 结构专属资源/蓝图、实用能力和生态差异经济。 | 奖励必须改变能力或下一步计划；新增持久状态当批冻结 save/terrain 迁移。 |
| 5 | P11E 敌人表现（Queued） | L | 多部件体素轮廓、关键动作和独特 Waystone 守护阶段。 | 新敌人必须同时带来新决策和新奖励；表现不能反向改变判定。 |
| 6 | P11F PLAYABILITY-RC（Queued） | M | 汇总逐批试玩、迁移、性能、恢复、包和延期证据。 | 固化构建/硬件、里程碑耗时、误操作、死亡与困惑时段；不自动 push 或打标签。 |

执行顺序固定为 `P11A -> P11B -> P11C -> P11D -> P11E -> P11F`。每批需要独立合同、
...
| M9 可玩性与操作体验 | P11A Planned | P11A 首先关闭输入动作冲突、鼠标/模式设置和 Physical Input v2；P11B-P11F 只有在前序真实试玩通过后才逐批开放。 |
