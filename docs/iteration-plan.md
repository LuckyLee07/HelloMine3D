...
| 下一产品阶段已选定 | 第 8 阶段以“更像完整游戏”为主线。K4、G2-G6、Alpha 开发检查点以及 N1-N6 均已完成；Release Candidate 的 H2-H3、Q1-Q3 已关闭，当前只剩 R3 真人输入/联动验收。R4 已由独立的原生 TSan 门禁关闭。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 14：Release Candidate（当前） | `R3` | H2-H3、Q1-Q3、R5 干净包和最终双配置自动门禁已完成；功能冻结后只继续取得真人输入证据。 | 双配置全门禁、soak 和真人输入全部通过。 | N1-N6 已冻结，只修缺陷。 |
...
| N6 产品体验（Done） | 键位、辅助选项、UI 统一、配方/目标可读性、正式音效与声音字幕；低密度音乐经评估后独立后置。 | 已交付；合同与取舍见 `product-experience-contract-v1.md`，后续只在 R3 或封板发现缺陷时修正。 |
