...
| 第 8 阶段 | 已启动，仍待 R3 封板 | `Q1` 已固定六类性能契约，`G1` 配方注册表已完成；`K1-K4`、`H1-H3`、`Q2-Q3`、`G2-G6` 仍按依赖推进 |
...
1. 仍需由真人在目标 Release 构建上完成 `R3`，同时关闭 `D2`、`D4` 和 `D6`；
   不依赖输入的 Q1 合同与 G1 基础数据层已先行完成，但这不替代阶段封板。
...
| 1：合同先行 | `K1`、`H1`、`Q1` | Q1 的本机可完成合同已落地；继续固定世界身份、崩溃产物/隐私边界和目标 Windows 正式预算。 |
