...
| 第 8 阶段 | 已启动，仍待 R3 封板 | `K1-K3` 世界目录/事务保存/备份恢复合同和 `G1` 配方注册表已完成，`Q1` 已固定六类性能契约；`K4`、`H1-H3`、`Q2-Q3`、`G2-G6` 仍按依赖推进 |
...
1. 仍需由真人在目标 Release 构建上完成 `R3`，同时关闭 `D2`、`D4` 和 `D6`；
   不依赖输入的 K1-K3/Q1 合同与 G1 基础数据层已先行完成，但这不替代阶段封板。
...
| 2：数据安全 | `K2-K3`、`Q2` | K2 事务保存与 K3 有界备份/验证恢复已完成；继续交付保存、备份、恢复和启动的分阶段耗时门禁。 |
...
| M6 可持续游玩与可靠发布 | 已启动，待 R3 与后续任务 | K1-K3 与 G1 已完成，Q1 的可移植合同已落地；继续交付 K4、H1-H3、Q2-Q3、G2-G6 及目标 Windows/真人证据。 |
