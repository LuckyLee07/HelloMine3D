...
边界，补齐一条有准备、有高潮、有结果、可以重玩的单机旅程。全部批次当前为 `Planned`，
只有实际启动时才进入 `docs/todolist.md` 的正式任务统计。

| 顺序 | 批次 | 核心结果 | 关键护栏 |
| ---- | ---- | -------- | -------- |
| 0 | RC0 基线收口 | 验证未推送提交 bundle，完成 R3，关闭 D2/D4/D6。 | 不改玩法/版本；自动 EXE 隐藏，只有真人 R3 前台执行；不 push。 |
| 1 | N7A 结局状态与文本键 | 独立世界结局事实、奖励 epoch、世界列表标记和语义化中英文 key 骨架。 | 不以目标耗尽推导胜利；保存迁移、恢复、fallback 和幂等。 |
| 2 | N7B 路标激活与胜利 | 激活路标，以现有敌人组成有界守卫战并显示胜利覆盖层。 | 保持 Playing、胜利后继续沙盒；不提前建立 Boss/GameMode。 |
| 3 | N8A 战斗可读性 | 前摇/恢复、方向性受击、击退和反馈。 | 小型显式 FSM、目标失效恢复、固定 tick 预算和调试快照。 |
| 4 | N8B 远程敌人与投射物 | 版本化战斗档案、远程威胁、瞬态投射物和最低限度格挡。 | 遮挡、寿命/距离/容量、死亡/卸载/重载清理；投射物不保存。 |
| 5 | N9A 确定性结构框架 | 复用 cell hash/`StructureBuilder` 固定结构归属、投影和覆盖规则。 | 不用 `std::rand`、不同步加载邻区块、不另设结构版本或通用 Manager。 |
| 6 | N9B 遗迹、营地与战利品 | 两类 seed 稳定地点带来探索风险与奖励。 | 初始战利品快照、持久箱子库存、terrain 身份和旧世界流送预算。 |
| 7 | N10 食物、冶炼与资源经济 | 扩大精确配方和资源交叉消费，并加入经济可达性校验。 | 守恒、无净增益循环、暂停/卸载；维持三槽熔炉，饥饿继续后置。 |
| 8 | N11A 难度档案 | 三档世界难度，创建时选择、暂停菜单修改并在下个固定 tick 生效。 | 旧世界 Normal；集中版本化参数和 metadata，不改变 terrain seed。 |
| 9 | N11B 胜利后事件 | 少量有界、可选的胜利后事件。 | P2 伸缩项，不做无限任务，不阻塞首个 Beta。 |
| 10 | N12A 本地化完成 | 完成中英文、字体、长文本、字幕和 Credits。 | key/fallback、资源/许可、截图、隐藏客户端与 R3。 |
| 11 | N12B 正式采样音效 | 建立采样音效 schema/后端并替换程序化占位声音。 | 缓存/并发、缺设备/文件降级、生命周期、许可和听感。 |
| 12 | N12C 低密度音乐 | 单通道流式环境音乐及暂停、淡入淡出和清理。 | P2 伸缩项；不引入 FMOD、多通道编排或实时 DSP。 |
| 13 | BETA-RC | 重建迁移、性能、长稳、崩溃、输入和发行包基线。 | 不放宽预算掩盖回归；记录新哈希、bundle 和本地标签，不 push。 |
...
非目标、最低回归和退出条件统一见 `docs/beta-gameplay-roadmap.md`。
...
| M7 Beta 单机游戏闭环 | 预排，未开始 | RC0 后按 N7A-N12C 交付结局、胜利、战斗、探索、经济、难度、本地化和正式音频；N11B/N12C 可后置，再由 BETA-RC 集中关闭迁移、性能、长稳、崩溃、输入和发行包。 |
