...
| 下一产品阶段已选定 | 第 8 阶段的 K4、G2-G6、Alpha 开发检查点以及 N1-N6 均已完成；Release Candidate 的 H2-H3、Q1-Q3 已关闭，当前只剩 R3 真人输入/联动验收。之后按 Stage 9 的 N7-N12 继续补胜利闭环、战斗、探索、资源经济、难度和产品表现，详细预排见 `docs/beta-gameplay-roadmap.md`。R4 已由独立的原生 TSan 门禁关闭。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 第 8 阶段 | 自动化闭环，待人工验收 | `K1-K4`、`G1-G6`、`N1-N6`、`H1-H3` 和 `Q1-Q3` 已完成；目标、成长、恢复、战斗掉落、生态探索、可访问体验、诊断、正式 Windows 预算和双档长稳均已串联；只等待 R3 真人输入。 |
| 第 9 阶段 | 预排，未开始 | `N7-N12` 与 `BETA-RC` 的范围、版本预案和回归触发器见 `docs/beta-gameplay-roadmap.md`；尚未计入当前 77 项总账。 |
...
| 14：Release Candidate（待人工验收） | `R3` | H2-H3、Q1-Q3、R5 干净包和最终双配置自动门禁已完成；功能冻结后只继续取得真人输入证据。 | 双配置全门禁、soak 和真人输入全部通过。 | N1-N6 已冻结，只修缺陷。 |
...
4. N6 使主要 UI 和输入稳定后，H2-H3 已验证 dump、脱敏 sidecar、离线符号、下次启动
   提示和干净包；Q1-Q3 预算与双档长稳也已关闭。R3 仍保留为真人证据。
5. 最终 Windows 双配置构建、存档故障、资源包、崩溃、性能、soak 和干净包自动门禁已
   通过；接下来只完成 R3 并同步关闭 D2/D4/D6。封板批只修缺陷，不继续增加内容。
...
| N6 产品体验（Done） | 键位、辅助选项、UI 统一、配方/目标可读性、基础程序化音效反馈与声音字幕；正式采样音效和低密度音乐经评估后独立后置。 | 已交付；合同与取舍见 `product-experience-contract-v1.md`，后续只在 R3 或封板发现缺陷时修正。 |

### 第 9 阶段：Beta 内容与首个胜利闭环

第 9 阶段不是扩大体素引擎底座，而是利用已经可靠的存档、目标、战斗、生态、诊断和性能
边界，先关闭真实首屏缺陷并建立视觉基线，再补齐一条有准备、有高潮、有结果、可以重玩的
单机旅程。`FS1` 已完成；其余批次只有实际启动时才进入 `docs/todolist.md`。
...
| 2 | FS3 体素美术与 HUD（Done） | 一致图集、图标化快捷栏、手持物与基础交互反馈。 | 539 项双配置世界回归、27 项资源包、47 项清单、来源记录、隐藏截图与焦点守卫通过；R3 留给 RC0。 |
| 3 | RC0 基线收口 | 验证未推送提交 bundle，完成 R3，关闭 D2/D4/D6。 | 在 FS3 后执行；自动 EXE 隐藏，只有真人 R3 前台；不 push。 |
| 4 | N7A 结局状态与文本键 | 独立世界结局事实、奖励 epoch、世界列表标记和语义化中英文 key 骨架。 | 不以目标耗尽推导胜利；保存迁移、恢复、fallback 和幂等。 |
| 5 | N7B 路标激活与胜利 | 激活路标，以现有敌人组成有界守卫战并显示胜利覆盖层。 | 保持 Playing、胜利后继续沙盒；不提前建立 Boss/GameMode。 |
| 6 | N8A 战斗可读性 | 前摇/恢复、方向性受击、击退和反馈。 | 小型显式 FSM、目标失效恢复、固定 tick 预算和调试快照。 |
| 7 | N8B 远程敌人与投射物 | 版本化战斗档案、远程威胁、瞬态投射物和最低限度格挡。 | 遮挡、寿命/距离/容量、死亡/卸载/重载清理；投射物不保存。 |
| 8 | N9A 确定性结构框架 | 复用 cell hash/`StructureBuilder` 固定结构归属、投影和覆盖规则。 | 不用 `std::rand`、不同步加载邻区块、不另设结构版本或通用 Manager。 |
| 9 | N9B 遗迹、营地与战利品 | 两类 seed 稳定地点带来探索风险与奖励。 | 初始战利品快照、持久箱子库存、terrain 身份和旧世界流送预算。 |
| 10 | N10 食物、冶炼与资源经济 | 扩大精确配方和资源交叉消费，并加入经济可达性校验。 | 守恒、无净增益循环、暂停/卸载；维持三槽熔炉，饥饿继续后置。 |
| 11 | N11A 难度档案 | 三档世界难度，创建时选择、暂停菜单修改并在下个固定 tick 生效。 | 旧世界 Normal；集中版本化参数和 metadata，不改变 terrain seed。 |
| 12 | N11B 胜利后事件 | 少量有界、可选的胜利后事件。 | P2 伸缩项，不做无限任务，不阻塞首个 Beta。 |
| 13 | N12A 本地化完成 | 完成中英文、字体、长文本、字幕和 Credits。 | key/fallback、资源/许可、截图、隐藏客户端与 R3。 |
| 14 | N12B 正式采样音效 | 建立采样音效 schema/后端并替换程序化占位声音。 | 缓存/并发、缺设备/文件降级、生命周期、许可和听感。 |
| 15 | N12C 低密度音乐 | 单通道流式环境音乐及暂停、淡入淡出和清理。 | P2 伸缩项；不引入 FMOD、多通道编排或实时 DSP。 |
| 16 | BETA-RC | 重建迁移、性能、长稳、崩溃、输入和发行包基线。 | 不放宽预算掩盖回归；记录新哈希、bundle 和本地标签，不 push。 |

N7A 预计在确有持久结局字段时把保存升级到 v9；N9 只有改变生成身份时才引入 terrain v3，
结构不另设独立版本；N11A/N12 只在载荷真实变化时升级保存、设置或音频定义。N11B/N12C 是
可后置伸缩项，其余 P0/P1 基础批次不因它们后置而删减。每批开始前分别建立合同，详细范围、
...
| M6 可持续游玩与可靠发布 | 自动化闭环，待 R3 | K1-K4、G1-G6、N1-N6、H1-H3、Q1-Q3 已完成；nominal/stress 正式长稳双 PASS，只等待 R3 真人输入。 |
| M7 Beta 单机游戏闭环 | FS1-FS3 已完成，RC0 待启动 | 已关闭首次进入和视觉/HUD 基线；下一步由 RC0/R3 固化人工起点，之后按 N7A-N12C 交付完整玩法闭环，最终由 BETA-RC 集中封板。 |
