...
| 10：食物与恢复（下一批） | `N3` | 严格食物定义、主动食用命令、恢复上限/冷却、面包配方、保存与目标接入。 | 食用成功才消耗；满血、暂停、死亡、冷却和 UI 占用不消耗；双配置定向回归通过。 | N2 已完成；先做恢复闭环，饥饿压力另行评估。 |
...
| N3 食物与恢复（下一批） | 让小麦和战斗损伤进入真实资源循环，先恢复、后评估饥饿压力。 | 玩家状态、库存守恒和 N1/N2 目标事件边界稳定。 |
...
| M6 可持续游玩与可靠发布 | 已启动，玩法闭环优先 | K1-K4、G1-G6、N1-N2、H1 已完成，H2 骨架、Q2 可移植采集与 Q1 合同已落地；下一批推进 N3 食物恢复，之后继续 N4-N6，并集中取得 R3、H2-H3、Q1-Q3 和发行证据。 |
