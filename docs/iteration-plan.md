...
| 9：熔炉与铁级成长（当前） | `N2` | 冶炼定义、熔炉容器、燃料/时间、铁锭和铁级工具/战斗用途。 | 每阶段执行守恒、暂停、卸载、保存重开和目标接入回归。 | N1 已完成；先锁数据/存档，再接 UI 和内容。 |
...
| N2 熔炉与铁级成长（当前） | 冶炼、燃料、铁锭和铁级工具形成第二段资源曲线。 | N1 已提供目标边界，G2/G3 守恒边界可复用。 |
