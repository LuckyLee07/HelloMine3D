...
| 下一产品阶段已选定 | 第 8 阶段、Stage 9/N7-N12 与 BETA-RC 工程均已完成；人工输入和产品体验继续延期。当前进入 Stage 10，先实施 V10A 顶点平滑光照/AO，再按 V10B1-B3、V10C-V10E 推进，详细范围见 `docs/visual-quality-roadmap.md`。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 第 10 阶段 | 进行中：V10A | `V10A`、`V10B1-B3`、`V10C-V10E` 与 `VISUAL-RC` 不计入当前 77 项总账；V10A 四角光照/AO、共享 solid 顶点、八场景对照和开发者检查已落地，Q1 性能例外尚未关闭，后续批次按视觉路线逐项启动。 |
...
| 顺序 | 批次 | 规模 | 核心结果 | 关键护栏 |
| ---- | ---- | ---- | -------- | -------- |
| 1 | V10A 顶点平滑光照与 AO（In Progress） | M/L | 四角邻域光照与接触遮蔽替代整面单亮度；共面 byte-identical solid 顶点通过索引复用。 | 保持 32 字节顶点布局；只有可重建 AO/光照才 greedy 合并；八图和开发者检查已通过，schema 3 Q1 仍为回归，需继续优化或取得明确批准。 |
| 2 | V10B1 材质与图集管线（Planned） | M | 参数化图集/tile/颜色管线。 | 旧图集像素兼容；不静默改变 V10A 光照曲线；shader 负例和 macOS Release 窗口冒烟。 |
| 3 | V10B2 原创材质资产（Planned） | L | 原创分面材质与世界/HUD/手持一致性。 | 生成脚本、来源、许可、manifest、资源包和近/中景开发者检查。 |
...
| M8 视觉质量升级 | In Progress：V10A | 按顶点光照/AO、B1-B3 材质拆分、大气云层、可选阴影、轻量后处理和 VISUAL-RC 顺序推进；当前 V10A 实现与自动回归已落地但 Q1/截图尚未关闭，开发者视觉检查与正式人工验收分层记录。 |
