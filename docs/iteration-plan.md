...
| 第 8 阶段 | 已规划，待 R3 封板后启动 | `K1-K4` 存档管理、`H1-H3` 崩溃诊断、`Q1-Q3` 性能预算、`G1-G6` 玩法成长与产品体验 |
...
1. 先由真人在目标 Release 构建上完成 `R3`，同时关闭 `D2`、`D4` 和 `D6`。
...
| 1：合同先行 | `K1`、`H1`、`Q1` | 先固定世界身份、崩溃产物/隐私边界和性能场景，不让后续 UI 与玩法自行定义基础协议。 |
| 2：数据安全 | `K2-K3`、`Q2` | 完成事务保存、备份恢复和保存/启动耗时采集；故障注入证明旧存档不被破坏。 |
| 3：玩家入口 | `K4`、`G1-G2` | 用世界列表和工作台把“选择世界—采集—制作”接入正常玩家流程。 |
...
| M6 可持续游玩与可靠发布 | 已规划，未开始 | 第 8 阶段以 `K/H/Q/G` 四条任务线交付长期存档、崩溃诊断、性能预算和制作/工具成长；`R3` 封板后开始。 |
