...
| 风险 | 说明 |
| ---- | ---- |
| 目标 Windows 更新门禁待复跑 | macOS 已验证版本化的 31 项 Xcode 工程图、Debug/Release 构建、十三个测试和真实窗口启动；Windows 上次证据早于 G1/K1-K3、Q2/H1 与当前工程图合同，需在目标机复跑更新后的 PowerShell 门禁和受控 minidump 验收。 |
| 输入仍有平台型证据缺口 | OIS 到控制器的数据边界已自动验证，但真实 Windows 键鼠仍需人工验收；原生 arm64 Apple Clang ThreadSanitizer 已关闭后台加载器的 R4 证据缺口。只剩 `R3`。 |
| 下一产品阶段已选定 | 第 8 阶段以“更像游戏”为主线，同时补齐存档管理、崩溃诊断和扩展性能预算。R3 仍是玩家功能的封板项；R4 已由独立的原生 TSan 门禁关闭。多人、脚本化 mod、热更新和新渲染后端仍未立项。 |
...
| 第 8 阶段 | 已启动，仍待 R3 封板 | `K1-K3` 世界目录/事务保存/备份恢复合同、`G1` 配方注册表和 `Q2` 可移植计时采集已完成，`Q1` 已固定六类性能契约；`H1` 的代码、便携合同和 Windows 验收脚本已落地但仍缺目标机 dump 证据，`K4`、`H2-H3`、`Q1/Q2` 目标 Windows 预算证据、`Q3`、`G2-G6` 仍按依赖推进 |
...
1. 仍需由真人在目标 Release 构建上完成 `R3`，同时关闭 `D2`、`D4` 和 `D6`；
   不依赖输入的 K1-K3/Q1 合同、Q2 可移植采集与 G1 基础数据层已先行完成，但这不替代阶段封板。
...
| 1：合同先行 | `K1`、`H1`、`Q1` | K1 与 Q1 的本机可完成合同已落地；H1 已固定崩溃产物/隐私边界并完成便携门禁，继续取得目标 Windows minidump 与正式性能预算。 |
| 2：数据安全 | `K2-K3`、`Q2` | K2 事务保存、K3 有界备份/验证恢复和 Q2 有界分阶段采集/成功失败夹具已完成；继续取得目标 Windows 正式预算与原生回归失败证据。 |
...
| M6 可持续游玩与可靠发布 | 已启动，待 R3 与后续任务 | K1-K3、Q2 可移植采集与 G1 已完成，Q1 的可移植合同及 H1 的实现/便携门禁已落地；继续取得 H1 目标 Windows dump 证据，并交付 K4、H2-H3、Q1/Q2 目标 Windows 预算证据、Q3、G2-G6 及真人证据。 |
