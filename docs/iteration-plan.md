...
| 第 10 阶段 | Todo：V10A | `V10A`、`V10B1-B3`、`V10C-V10E` 与 `VISUAL-RC` 不计入当前 77 项总账；当前先补顶点平滑光照/AO，后续批次按视觉路线逐项启动。 |
...
| 顺序 | 批次 | 规模 | 核心结果 | 关键护栏 |
| ---- | ---- | ---- | -------- | -------- |
| 1 | V10A 顶点平滑光照与 AO（Todo） | M/L | 四角邻域光照与接触遮蔽替代整面单亮度。 | 保持 32 字节顶点布局；只有可重建 AO/光照才 greedy 合并；schema 3 补充比较、短 Q3、八图和开发者检查。 |
...
| M8 视觉质量升级 | Todo：V10A | 按顶点光照/AO、B1-B3 材质拆分、大气云层、可选阴影、轻量后处理和 VISUAL-RC 顺序推进；当前只启动 V10A，开发者视觉检查与正式人工验收分层记录。 |
