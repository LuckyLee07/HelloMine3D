...
| B3 native Xcode gate | `bash scripts/verify_xcode.sh` on macOS | Pending a macOS host. The script requires Debug/Release client and test builds, ten test executions, two validation-only bootstraps and two real-window three-frame launches before printing `status=PASS`. |
