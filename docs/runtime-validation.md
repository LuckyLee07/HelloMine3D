...
| ----- | ------- | ------ |
| Headless test targets | The eight `HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries enumerated by `scripts\verify_build.ps1` | All eight pass in Debug and Release on macOS; the world target reports `checks=338 failures=0`, while soak, resource-pack and recipe targets also pass in both configurations. The updated eight-target Windows graph requires its next native run. |
| World runtime smoke | `bin\HelloMine3DWorldRuntimeSmoke.exe` | `checks=338 failures=0`; eight current V2/W1 assertions cover input-sampling idempotence, held sneak, camera interpolation, normalized diagonal/sprint movement, sky/fog horizon continuity and opposite noon/midnight sun directions (Debug/Release, 2026-08-16). |
...
| B3 Xcode generation preflight | `powershell -NoProfile -ExecutionPolicy Bypass -File tools\validate_xcode_generation.ps1` | The contract now expects 26 macOS Xcode projects, including the G1 recipe smoke, and 148 workspace/verifier checks. The verifier parallelizes target builds and rejects duplicate project references, deprecated manual ordering and every first-party compiler warning. The last recorded Windows-hosted run was the preceding 25-project/142-check graph; the updated PowerShell preflight requires the next Windows run. Native Debug/Release builds pass on macOS (updated 2026-08-16). |
| B3 native Xcode gate | `bash scripts/verify_xcode.sh` on macOS | Apple M1 Pro completed Debug/Release client and eight-test builds, sixteen test executions, two validation-only bootstraps and two real Cocoa 120-frame launches. Both world runs report `checks=338 failures=0`; both window probes persist surface contact at `Y=66`. Parallel target builds emit no duplicate-reference, manual-order or first-party compiler warnings, and the gate reports `status=PASS` under `build/xcode-validation-20260816225230` (2026-08-16). |
...
| D6 playable vertical slice | `bin\HelloMine3DWorldRuntimeSmoke.exe` (`casePlayableVerticalSlice`); `tools\run_render_capture.ps1 -ShowVerticalSliceFixture`; R1 comparison | Nine assertions drive crop, chest, natural population, combat, physical loot pickup, replanting, save and relaunch through normal gameplay APIs after fixture creation. The current full world run reports `checks=338 failures=0`. `validation-playable-loop.png` visibly records the open chest with Wheat, hotbar materials and staged world. Base/candidate P95 is `20.956/21.122 ms`, P99 is `25.926/25.178 ms`, and R1 returns `PASS`. Physical input remains R3, so D6 remains `Verify` (updated 2026-08-16). |
