...
| B3 | Blocked | Verify the macOS path. | Windows-side closure removed unconditional `WIN32` macros from vendored dependency projects and added `tools/validate_xcode_generation.ps1`. The generated workspace contains the client, five tests and the Ogre/OIS graph; 69 checks require Cocoa/OIS/GL sources and frameworks while rejecting Win32 macros and build-phase sources. This host cannot run Xcode. | Static generation passes. Final acceptance remains `./xcode.sh`, native `xcodebuild` for Debug/Release and a client launch on macOS. |
