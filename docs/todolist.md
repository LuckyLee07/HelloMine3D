...
| B3 | Done | Verify the macOS path. | Platform macros and header paths are scoped per host; Ogre core/GLSupport receive their OSX headers, OIS receives Cocoa headers, and the stale WiiMote demo is excluded. A shared version-1 contract fixes the exact 26-project workspace/on-disk inventory. The gate parses every PBX group and `ProjectRef`, then parallelizes quiet builds and rejects stale/missing projects, duplicate/multi-group references, deprecated manual ordering or any first-party compiler warning. | Nine graph/contract fixtures pass, and the actual graph contains exactly 26 projects, 327 groups, 2,470 memberships and 54 cross-project references. The full Apple M1 Pro Debug/Release client plus eight-test gate, all 16 executions, two validation-only starts and two real Cocoa starts report `status=PASS` under `build/xcode-validation-20260816230649`; target-Windows PowerShell consumes the same contract on its next run. |
