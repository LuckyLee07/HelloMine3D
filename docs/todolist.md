...
| 玩法数据层 | G1-G6、N1-N6、N7A-N12C 与 BETA-RC 工程封板均已完成；当前没有排队中的开发批次。 |
...
与 `N12A/N12B/N12C`、`BETA-RC` 工程封板当前为 `Done`，没有后续开发批次排队。
...
| BETA-RC 集中封板 | Done（工程） | P0 | 全迁移、正式性能、双档长稳、崩溃符号和新干净包已经复核。 | 六类 Q1、两档 1800 秒 Q3、自动 R3 预检和 bundle 校验完成；真人 R3 延后，因此不建本地标签、不 push。 |
...
已经闭环，BETA-RC 工程封板也已完成；R3 维持部分自测、其余延期。Stage 9 当前没有后续
开发批次，但以上新增批次仍不回写拆分前 77 项历史总账。
