...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 173 assertions, including block-use events, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=173 failures=0` |
