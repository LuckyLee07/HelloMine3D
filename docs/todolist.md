...
| 玩法数据层 | G1-G6、N1-N6、N7A/N7B、N8A/N8B、N9A/N9B、N10、N11A/N11B 与 N12A/N12B/N12C 已完成；当前下一批为 BETA-RC 工程封板。 |
...
| 当前 Windows 门禁 | 2026-08-26 N12C 完整通过 64 项资源、38 个性能夹具、R3 部分自测/其余延期记录、VS2017/v141 双配置客户端及 699/699 世界、38/38 资源包目标、Debug/Release 0 错误全量重建、十三目标测试、隐藏校验/真实三帧客户端、131,321 字节后台受控 dump 和 84 文件干净发行包；ZIP SHA-256 为 `DABE356BC511A110838DC1CD98BEE9392D0FC03FFBA1D40DB8271B6BD5392E2A`。 |

## 进度摘要

拆分前总账共有 77 个正式任务；历史总账仍为 73 个 `Done`、4 个 `Verify`。新增的 Stage 9
批次不回写该统计：`FS1-FS3`、`N7A/N7B`、`N8A/N8B`、`N9A/N9B`、`N10`、`N11A/N11B`
与 `N12A/N12B/N12C` 当前为 `Done`，下一批为 `BETA-RC`。
R3 已做部分非正式真人自测，尚未形成十二项正式记录，其余按用户决定延后且不阻塞后续玩法开发。

| 类型 | 数量 | 任务 |
| ---- | ---- | ---- |
| 已完成的第 8 阶段正式任务 | 16 | K1-K4、G1-G6、H1-H3、Q1-Q3 |
| Stage 9 已完成 | 15 | FS1 首次进入正确性、FS2 天空/水面/颜色、FS3 体素美术/HUD、N7A 结局状态与文本键基础、N7B 路标胜利闭环、N8A 战斗可读性、N8B 远程敌人与投射物、N9A 确定性结构框架、N9B 遗迹/营地与生态战利品、N10 食物/冶炼/资源经济、N11A 难度档案、N11B 胜利后事件、N12A 本地化完成、N12B 正式采样音效、N12C 低密度音乐 |
...
下列批次已经落成可回归路线，`FS1-FS3`、`N7A/N7B`、`N8A/N8B`、`N9A/N9B`、`N10`、`N11A/N11B` 与 `N12A/N12B` 已转成
...
| N12C 低密度音乐 | Done | P2 | 已交付单通道流式环境音乐及暂停、停止、淡入淡出、降级和清理。 | settings v4、1 条原创 20 秒曲目、352 个双语 key、64 项清单、双配置 699/699 世界与 38/38 资源包、真实 WaveOut、完整门禁和同身份 Q1 通过；真人听感随 R3 延期。 |
...
N11B -> N12A -> N12B -> N12C -> BETA-RC`；RC0/R3 的剩余人工项单独延后。N11B/N12C 均已
...
当前进入 BETA-RC 工程封板，但不回写拆分前 77 项历史总账。
