...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 253 assertions, including deterministic time-of-day values, configured-seed determinism, generated runtime-config ownership, strict block-definition and shape parsing, block-use events, behavior dispatch, metadata-backed behavior and bounded random ticks, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, resource-driven non-cube shapes, deterministic cave generation, load-order-independent cross-chunk structures, mob chase and damage immunity, sunlight/block-light storage, local relighting and transparent-pass rules, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=253 failures=0` |
...
| ID | Status | Task | Validation |
| -- | ------ | ---- | ---------- |
| W1 | In Progress | Add a simple time-of-day and fog parameter channel without rewriting stored sunlight. The portable 24,000-tick model supplies daylight, sky tint, fog colour and fog density without touching any stored block-light or sunlight values. Ogre applies the values to terrain, water, flora, actors, the skybox and scene fog; debug UI and performance captures expose them. `HELLOMINE3D_WORLD_TIME` / `run_render_capture.ps1 -WorldTime` pins a repeatable frame. | Eight `W1/*` assertions cover anchors, wrapping, value bounds, twilight continuity, automation override and stats. The Windows Debug/Release gate passes all tests (`checks=253 failures=0`). A same-scene noon/midnight hardware comparison remains before Done. |
| W2 | Todo | Surface resource/bootstrap failures in a user-facing startup error view while preserving complete stderr diagnostics. | Inject missing shader, texture and block fixtures; each names the failed resource, returns non-zero and presents the Windows-visible error path without hiding the log. |
| W3 | Todo | Add a generated resource manifest before attempting resource packs. | The manifest covers every checked block, shape, shader, texture, font and runtime template; stale or missing entries fail independently of the live asset tree. |
| W4 | Todo | Measure the current terrain vertex layout and compress it only when the bandwidth/memory evidence justifies the added shader complexity. | Record bytes per vertex and resident terrain-buffer estimates. Any layout change must preserve every CPU stream assertion, hardware rendering and the performance baseline. |
