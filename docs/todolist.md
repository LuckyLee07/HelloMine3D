...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 107 assertions covering S0-S6, fixed ticks, synthetic player input, height-map edits, headless atlas coordinates, and the mesh halo snapshot. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=107 failures=0` |
