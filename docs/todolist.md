...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 318 assertions, including deterministic time-of-day values, terrain-buffer layout metrics, configured-seed determinism, generated runtime-config ownership, strict block-definition and shape parsing, block-use events, behavior dispatch, metadata-backed behavior and bounded random ticks, stateful-block ownership, chest transfers and reload, bounded natural-mob population and deduplicated restore, actor/block targeting priority, combat damage, death/drop ordering and player respawn, the complete wheat seed/plant/grow/harvest/replant loop, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, resource-driven non-cube shapes, deterministic cave generation, load-order-independent cross-chunk structures, mob chase and damage immunity, sunlight/block-light storage, local relighting and transparent-pass rules, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=318 failures=0` |
...
| Active next scope | The Windows sandbox foundation, R1, D1, D3 and D5 are closed. D2 and D4 implementation, automated coverage and hardware captures are complete and await the R3 physical-input record. R2-R3 are next, followed by D6, R5 and X1-X3. B3 and R4 remain host-deferred. | Milestones D, R and X below |
