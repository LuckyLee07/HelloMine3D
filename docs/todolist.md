...
| ---- | ------------- | ------------ |
| Build system | Premake is the sole project generator. Platform verification wrappers generate one `HelloMine3D` client, eight test targets, and the Ogre/OIS static-library graph, then rebuild and test Debug and Release. | `scripts\verify_build.ps1` on Windows or `scripts/verify_build.sh` on Linux/macOS |
...
| Test targets | Eight first-party headless targets cover coordinates, mesh dirty propagation, save compatibility, actor lifecycle, the 338-check world stack, deterministic soak scheduling, resource-pack resolution and the strict recipe registry. | The eight `HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries listed in `scripts\verify_build.ps1` |
...
| B2 | Done | Document and script build verification (old M0.4). | `verify_build.ps1` uses bundled Premake, locates VS2022 MSBuild, rejects OIS demo sources, builds Debug/Release x64 and runs all eight current tests after each build. `verify_build.sh` applies the same contract through Premake/Make on Linux and macOS. README lists one command and its host prerequisites per path. | The last recorded Windows wrapper run completed both full builds and fourteen executions for the then-seven-test graph. The current eight-test contract, including G1 recipe validation, requires the next Windows run; the matching macOS Xcode gate passes both configurations. |
| B3 | Done | Verify the macOS path. | Platform macros and header paths are scoped per host; Ogre core/GLSupport receive their OSX headers, OIS receives Cocoa headers, and the stale WiiMote demo is excluded. Redundant `dependson` entries no longer duplicate linked project references. `scripts/verify_xcode.sh` parallelizes quiet builds, preserves per-target logs and fails on duplicate project references, deprecated manual ordering or any first-party compiler warning. | On 2026-08-16 an Apple M1 Pro completed Debug/Release client plus eight-test builds, all 16 test executions (`checks=338 failures=0` in both world runs), two validation-only starts and two real Cocoa 120-frame starts with no gated build warnings. Both window probes persisted surface contact at `Y=66`; the gate reports `[XCODE_VERIFY] status=PASS` under `build/xcode-validation-20260816225230`. |
...
| B4 | Done | Remove dead build artifacts. | Removed ignored outputs for the deleted `HelloMine3DFoundationRuntimeSmoke` target and the pre-E5 standalone `HelloMine3DOgreBootstrap` target. Both verification wrappers now reject any `HelloMine3D*` executable outside the current client-plus-eight-tests allowlist. | The current macOS gate contains exactly the client and eight test executables. The updated Windows allowlist includes G1 RecipeSmoke and will be revalidated on the next target-Windows run. |
...
| Headless tests | eight `bin\HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries | All code changes |
