...
| 玩法数据层 | G1-G6 与 N1 已完成：严格配方、制作、木/石工具成长、暂停设置、基础音频、十步 Alpha 旅程和数据驱动目标已经串联；当前进入熔炉/铁级成长。 |
| 性能观测 | Q1 七类场景合同、Q2 有界阶段计时和可选 Tracy 已落地；Alpha 隐藏场景已有批准基线，其余六类正式 Windows 预算仍待批准。 |
| 崩溃诊断 | H1 已闭环；H2 已交付严格脱敏 sidecar 和精确 PDB 身份校验的单帧离线符号骨架，默认不上传。 |
...
拆分前总账共有 77 个正式任务；当前为 68 个 `Done`、4 个 `Verify`、3 个 `Doing`、
2 个 `Todo`。第 8 阶段的 16 项任务中，K1-K4、G1-G6 和 H1 已完成；H2、Q1、Q2
正在推进；H3、Q3 尚未开始。Alpha 开发检查点和 N1 已关闭，当前进入 N2 熔炉、冶炼
与铁级成长。

| 类型 | 数量 | 任务 |
| ---- | ---- | ---- |
| 已完成的第 8 阶段底座与玩法 | 11 | K1、K2、K3、K4、G1、G2、G3、G4、G5、G6、H1 |
| 正在实现或尚未正式闭环 | 3 | H2、Q1、Q2 |
...
| N2 | Doing | P0 | 让煤和铁矿进入真实用途，形成熔炉、燃料、铁锭和铁级工具的第二段成长。 | 严格冶炼定义；输入/燃料/输出守恒；暂停、卸载、保存重开确定；铁级产物接入目标链和真实采集/战斗用途。 | N1、G2、G3 |
...
| H2 | Doing | P1 | 版本化、脱敏 sidecar 和离线符号。 | Alpha 检查点已完成严格 schema、路径脱敏、匹配单帧解析和错误 PDB 拒绝；后续补任意 dump 完整栈和符号归档体验。 |
| H3 | Todo | P2 | 干净包和下次启动的本地崩溃提示。 | 发行包不含符号/旧崩溃文件；下次启动可本地打开、复制或忽略，不联网。 |
| Q1 | Doing | P1 | 冷启动、进入世界、保存、恢复、快速移动、规模玩法和 Alpha 综合流程的版本化性能场景。 | Alpha 综合场景已批准；继续采集并批准其余六个目标 Windows Release 场景。 |
| Q2 | Doing | P1 | 启动/目录/世界进入/保存/备份/恢复的有界阶段计时。 | Alpha 场景预算已证明启动、进入和保存通过；其余 Q1 场景继续闭环。 |
...
| 8：内容扩展（当前 N2） | `N1-N6` | N1 目标引导已完成；当前做熔炉/铁级，再推进食物恢复、战斗、生态和产品体验。 | 每项内容必须进入目标、成长或战斗循环；N2 必须复用 N1 目标和 G2/G3 守恒边界。 |
...
直播 DSP 或平台资源层。Alpha 之后的目标系统已由 N1 交付；熔炉/铁级、食物、战斗、生态
和体验批次按 `docs/game-development-roadmap.md` 继续推进，但不回写拆分前 77 项历史总账。
