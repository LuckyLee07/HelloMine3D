...
当前唯一计划开发的批次是 P11-0 世界光源与洞穴照明；其余批次只按固定顺序排队，不同时开工。

当前采用“玩法闭环优先”策略。与当前批次直接相关的真人输入、短试玩、编译、定向自动测试、
数据守恒和存档兼容必须当批执行；macOS 原生、完整性能长稳和最终发行包可以在对应封板批次
集中验收，但必须保持真实 `Verify` 状态。
...
| 玩法数据层 | G1-G6、N1-N6、N7A-N12C、BETA-RC 与 Stage 10/VISUAL-RC 均已完成 Windows 工程范围；Stage 11 已选定，P11-0 是下一开发批次。 |
...
D4/D6 和 Stage 9/10 的全部人工体验。Stage 11 新增九个不回写历史统计的批次：P11-0 为
`Planned`，其余八批为 `Queued`。2026-08-28 的内容与视觉复核在原有六批之间插入了 P11-0、
P11-1 和 P11-2 三个内容密度批次，理由是原有批次的退出条件依赖世界里并不存在的内容。
P11A 仍必须建立 Physical Input v2，并与视觉/双语/听感验收分开；不能因代码完成而自动关闭
D2/D4/D6/R3。
...
| Stage 11 下一批 | 1 | P11-0 世界光源与洞穴照明（Planned） |
| Stage 11 已排队 | 8 | P11A 核心操作手感、P11B 动作反馈、P11-1 建造材料与工具分化、P11C 前 30 分钟、P11D 探索奖励、P11-2 地形轮廓与洞口、P11E 敌人表现、P11F PLAYABILITY-RC |
...
Stage 11 的固定顺序为
`P11-0 -> P11A -> P11B -> P11-1 -> P11C -> P11D -> P11-2 -> P11E -> P11F`。当前只开放
P11-0；后续批次必须在前一批通过真实试玩退出条件后，逐个从 `Queued` 切换为 `Planned`。
权威范围、版本预案、试玩记录字段和非目标见 `docs/playability-experience-roadmap.md`。

| 批次 | 状态 | 优先级 | 计划交付 | 退出边界 |
| ---- | ---- | ------ | -------- | -------- |
| P11-0 世界光源与洞穴照明 | Planned | P0 | 可制作火把、修正 `Rose` 的遗留发射值、metadata 驱动发射与燃烧中熔炉发光、发射翻转的局部重光照。 | 2x2 制作区即可做出火把；洞穴与夜晚可正常游玩；熔炉点火/加工受阻或恢复/燃料耗尽只在 metadata 真正翻转时重光照；暂停保持亮度、卸载不改状态；除 `Rose`/`Furnace` 外所有方块光照输出逐字节不变；不升 save/terrain/settings 版本。合同见 `docs/world-light-source-contract-v1.md`。 |
| P11A 核心操作手感 | Queued | P0 | 分离使用/放置/格挡，扩展鼠标重绑定，统一灵敏度增量与焦点/捕获恢复，支持冲刺/潜行按住或切换。 | Physical Input v2 在目标 Windows 机器真实覆盖移动、视角、世界动作、UI、焦点与重启；自动合同、VS2017/v141 双配置和受影响回归通过；无串键、焦点泄漏或分辨率/帧率相关漂移。 |
| P11B 动作反馈与游戏汁水 | Queued | P0 | 手/工具动作、裂纹、粒子、命中/受击/冷却、拾取和音频微变体。 | Release 真人观察能读懂动作和危险窗口；镜头反馈可调/可关；不破坏 HUD 和低配回退。 |
