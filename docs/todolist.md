...
| ---- | ------------- | ------------ |
| Build system | Premake is the sole project generator. `vs2022.bat` generates the six original first-party targets, the E2 Ogre bootstrap target, and the Ogre/OIS static-library graph for VS2022 x64. | `vs2022.bat`, `MSBuild build\HelloMine3D.sln /p:Configuration=Debug /p:Platform=x64` |
| Dependencies | SFML 3 remains the active client path while the parallel Ogre bootstrap uses Ogre 1.10, GL3Plus, OIS and the minimum image/font/archive dependency closure built from vendored source. No checked-in binaries. | Debug and Release full-solution builds |
...
| E1 | Verify | Bring Ogre into the build (minimum subset: `ogre3d`, `ogre3d_glsupport`, `ogre3d_gl3plus`, FreeImage chain, dedicated Ogre FreeType, zlib, zzip and OIS). The solution is unified on x64, C++17, static CRT and MBCS while the game remains on the SFML path. | Premake generation, Debug/Release full builds and all five test binaries pass (`checks=123 failures=0`). The render probe reaches the unchanged SFML startup path but this session creates only GDI Generic OpenGL 1.1, so the hardware screenshot and comparable performance capture remain pending. |
| E2 | Verify | A parallel `HelloMine3DOgreBootstrap` target now owns `Root`/`RenderWindow`/`SceneManager`/`Camera`/`FrameListener`, resource config, OIS input, the built-in skybox, and a free-fly camera. The SFML client and five tests remain unchanged. | Debug/Release full builds and all five tests pass (`checks=123 failures=0`). Validation-only startup registers GL3Plus, two resource locations, and OIS. The window probe reaches GL3Plus context creation but this session exposes only GDI Generic OpenGL below 3.0, so a hardware-backed skybox and input check remains pending. |
| E3 | Verify | `ChunkSectionRenderable` now derives from Ogre's `SimpleRenderable` (`MovableObject` + `Renderable`), owns interleaved position/UV/cardinal-light vertex and 32-bit index buffers, uses a local section AABB, and is attached to one scene node per section. The unchanged `ChunkMeshBuilder` feeds the new terrain material; `DefaultPack.png` is loaded through the Ogre resource group with `filtering none`. | Debug/Release generate a fixed-seed real world and validate 10 solid sections, 14,460 vertices and 21,690 indices before any GPU call. Full builds and all five tests pass (`checks=123 failures=0`). The window probe still stops at the session's GDI Generic OpenGL limit, so hardware terrain capture and the comparable performance baseline remain pending. |
| E4 | In progress | Water and flora now use dedicated animated Ogre vertex programs, shared alpha-tested atlas sampling, and separate render queues (flora 60, transparent water 80). Ogre screenshot capture uses `RenderWindow::writeContentsToFile`, performance sampling runs from the frame listener, and both scripts accept `-Backend Ogre`. HUD and ImGui remain. | At seed `20260809` and position `264 96 8`, Debug/Release validate 19 solid, 3 water and 13 flora sections with non-empty, internally consistent CPU meshes. Capture configuration validates three scheduled targets, full builds and all five tests pass (`checks=123 failures=0`); hardware material appearance and actual Ogre PNG/CSV output remain pending. |
