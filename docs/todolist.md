...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 183 assertions, including configured-seed determinism, generated runtime-config ownership, strict block-definition diagnostics, block-use events, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=183 failures=0` |
...
| A2 | Done | Strengthen `.block` parsing diagnostics. | `BlockData` now requires every field, rejects missing values, unknown/duplicate keys, non-integers, invalid booleans/enums, block ids outside the registry and atlas coordinates outside the 16x16 sheet. `BlockDatabase` additionally checks declared ids against registration and rejects ids reused by another source file. | Four `A2/*` fixtures cover a missing key, invalid enum, out-of-range atlas coordinate and duplicate id; every diagnostic contains the full file path and exact key. Debug/Release full builds, all five tests and validation-only startup pass (`checks=183 failures=0`). |
...
| B5 | Verify | Fix the render capture script race. | Runtime polling now checks every expected PNG for non-zero content before interpreting process exit. A clean exit receives a two-second filesystem-visibility grace period; non-zero exits and genuinely missing files still fail. `-ValidateCapturePolling` reproduces the former ordering race without a GPU. | Ten deterministic polling runs pass after observing an already-exited clean process with both files present. This session only exposes GDI Generic OpenGL 1.1, so ten consecutive real GL3+ captures remain pending. |
