...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 132 assertions, including versioned renderer mesh snapshots, stale-upload rejection and actor render snapshots. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=132 failures=0` |
