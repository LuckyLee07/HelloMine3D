...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 110 assertions covering S0-S6, fixed ticks, synthetic player input, height-map edits, loader-thread stress, headless atlas coordinates, and the mesh halo snapshot. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=110 failures=0` |
