...
| 玩法数据层 | G1-G6、N1 与 N2 已完成：严格配方、制作、木/石/铁工具成长、暂停设置、基础音频、十步 Alpha 旅程、数据驱动目标和熔炉冶炼已经串联；下一批进入食物与恢复。 |
...
正在推进；H3、Q3 尚未开始。Alpha 开发检查点、N1 和 N2 已关闭，下一批进入 N3 食物
与恢复闭环。
...
| N3 | Todo | P0 | 让现有小麦和战斗损伤进入真实恢复循环，先完成主动食物，再根据实测决定是否需要饥饿压力。 | 严格食物定义；成功食用才原子消耗并恢复；满血、暂停、死亡、冷却和 UI 占用均不消耗；状态保存、目标接入和回归通过。 | N2、D4、D5 |
...
| 8：内容扩展（下一批 N3） | `N1-N6` | N1 目标引导和 N2 熔炉/铁级已完成；下一步做食物恢复，再推进战斗、生态和产品体验。 | 每项内容必须进入目标、成长或战斗循环；N3 必须复用 N1 目标、玩家状态和库存守恒边界。 |
...
熔炉/铁级成长已由 N1/N2 交付；食物、战斗、生态和体验批次按
