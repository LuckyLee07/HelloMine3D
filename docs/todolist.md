...
| Test targets | Thirteen first-party headless targets cover coordinates, mesh dirty propagation, save compatibility, actor lifecycle, the 346-check world stack, deterministic soak scheduling, resource-pack resolution, strict recipes, K1 catalogue, K2 transactional publication, K3 backup/restore, Q2 bounded operation timing and H1 crash policy. | The thirteen `HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries listed in `scripts\verify_build.ps1` |
...
| Active next scope | Close R3 with a human first, which also closes D2, D4 and D6. Stage 8 has delivered K1-K3, Q1's portable contract, Q2's portable instrumentation/fixtures and G1. H1's implementation and portable gate are present; its target-Windows dump evidence, K4, H2-H3, Q1/Q2 Windows budgets, Q3 and G2-G6 remain. B3 is Done; R4 remains host-deferred. | Milestones K, H, Q and G below; ordering in `Recommended Order` |
...
| ID | Status | Priority | Task | Depends on | Required acceptance |
| -- | ------ | -------- | ---- | ---------- | ------------------- |
| H1 | Doing | P1 | Integrate a minimal Windows minidump path, preferring Google Breakpad after an integration audit. The audit selected the already-linked Windows SDK DbgHelp backend and records Breakpad as not selected. The handler installs before Ogre, writes only to a separately validated crash directory, uses a pre-created writer thread, a 15-second wait and a one-shot recursion guard, and has no upload path. | B2, R5 | The portable policy/backend smoke passes 12/0 in Debug and Release, and the current full macOS gate proves ordinary validation/window starts remain green. `tools\validate_crash_diagnostics.ps1` now requires the controlled Release process to save the active world, exit non-zero, emit exactly one non-empty dump, reopen the world and leave no pending save state. H1 remains open until that target-Windows harness passes; `docs/crash-diagnostics-contract-v1.md` freezes the backend audit and evidence boundary. |
