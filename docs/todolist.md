...
| ---- | ------------- | ------------ |
| Build system | Premake is the sole project generator. Platform verification wrappers generate one `HelloMine3D` client, nine test targets, and the Ogre/OIS static-library graph, then rebuild and test Debug and Release. | `scripts\verify_build.ps1` on Windows or `scripts/verify_build.sh` on Linux/macOS |
...
| Test targets | Nine first-party headless targets cover coordinates, mesh dirty propagation, save compatibility, actor lifecycle, the 341-check world stack, deterministic soak scheduling, resource-pack resolution, strict recipes and the K1 world catalogue. | The nine `HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries listed in `scripts\verify_build.ps1` |
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 341 assertions. Its D6 scenario chains normal gameplay actions after one fixed fixture; K1 additionally covers version-3 identity creation, quoted names and stable ids across relaunch. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=341 failures=0` |
...
| Active next scope | Close R3 with a human first, which also closes D2, D4 and D6. Stage 8 has delivered K1, Q1's portable contract and G1; K2-K4, H1-H3, Q1's target-Windows evidence plus Q2-Q3 and G2-G6 remain. B3 is Done; R4 remains host-deferred. | Milestones K, H, Q and G below; ordering in `Recommended Order` |
...
| B2 | Done | Document and script build verification (old M0.4). | `verify_build.ps1` uses bundled Premake, locates VS2022 MSBuild, rejects OIS demo sources, builds Debug/Release x64 and runs all nine current tests after each build. `verify_build.sh` applies the same contract through Premake/Make on Linux and macOS. README lists one command and its host prerequisites per path. | The last recorded Windows wrapper run completed both full builds and fourteen executions for the then-seven-test graph. The current nine-test contract, including G1 recipes and the K1 catalogue, requires the next Windows run; the matching macOS Xcode gate is the native acceptance path. |
| B3 | Done | Verify the macOS path. | Platform macros and header paths are scoped per host; Ogre core/GLSupport receive their OSX headers, OIS receives Cocoa headers, and the stale WiiMote demo is excluded. A shared version-1 contract fixes the exact 27-project workspace/on-disk inventory. The gate parses every PBX group and `ProjectRef`, then parallelizes quiet builds and rejects stale/missing projects, duplicate/multi-group references, deprecated manual ordering or any first-party compiler warning. | Nine graph/contract fixtures pass, and the actual graph contains exactly 27 projects, 332 groups, 2,484 memberships and 54 cross-project references. The full Apple M1 Pro Debug/Release client plus nine-test gate, all 18 executions, two validation-only starts and two real Cocoa starts report `status=PASS` under `build/xcode-validation-20260816233050`; target-Windows PowerShell consumes the same contract on its next run. |
...
| B4 | Done | Remove dead build artifacts. | Removed ignored outputs for the deleted `HelloMine3DFoundationRuntimeSmoke` target and the pre-E5 standalone `HelloMine3DOgreBootstrap` target. Both verification wrappers now reject any `HelloMine3D*` executable outside the current client-plus-nine-tests allowlist. | The current graph contains exactly the client and nine test executables. The updated Windows allowlist includes G1 RecipeSmoke and K1 WorldCatalogueSmoke and will be revalidated on the next target-Windows run. |
