...
Last refreshed 2026-08-16. Update this when a milestone closes or the active
execution scope changes.

| Area | Current state | Verification |
| ---- | ------------- | ------------ |
| Build system | Premake is the sole project generator. Platform verification wrappers generate one `HelloMine3D` client, seven test targets, and the Ogre/OIS static-library graph, then rebuild and test Debug and Release. | `scripts\verify_build.ps1` on Windows or `scripts/verify_build.sh` on Linux/macOS |
...
| Test targets | Seven first-party headless targets cover coordinates, mesh dirty propagation, save compatibility, actor lifecycle, the 330-check world stack, deterministic soak scheduling and resource-pack resolution. | The seven `HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries listed in `scripts\verify_build.ps1` |
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 330 assertions. Its D6 scenario chains normal gameplay actions after one fixed fixture; V2 additionally covers swept falling collision, stable ground contact and jumping after rest. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=330 failures=0` |
| Render smoke | Ogre terrain, skybox, glass, water, leaves, flora, actors, ore textures, outline and HUD are implemented and validation-only startup passes. GTX 1050 Ti / OpenGL 4.6 captures cover the deterministic visual fixtures. | `docs/render-regression-smoke.md`, `docs/screenshots/validation-*.png` |
...
| Active next scope | Close R3 with a human first, which also closes D2, D4 and D6. The planned 16-item stage-8 scope then continues with K1-K4 durable world management, H1-H3 local crash diagnostics, Q1-Q3 expanded performance budgets and G1-G6 gameplay progression/product UX. B3 is Done; R4 remains host-deferred and does not block Windows work. | Milestones K, H, Q and G below; ordering in `Recommended Order` |
...
| E2 | Done | The Ogre shell owns `Root`/`RenderWindow`/`SceneManager`/`Camera`/`FrameListener`, resource config, OIS input and the built-in skybox. It was introduced as a parallel bootstrap and became `HelloMine3D.exe` in E5. | Validation-only startup registers GL3Plus, two resource locations and OIS; V2 validates the controller seam. The hardware runtime loads all six `sky.png` cube faces and `docs/screenshots/validation-skybox-panel-outline.png` confirms the skybox. |
...
| ID | Status | Task | Implementation notes | Validation |
| -- | ------ | ---- | -------------------- | ---------- |
| A1 | Done | Add `scripts/check_assets.sh`. | The Bash script derives block definitions from `BlockDatabase.cpp`, shape resources from `.block` files, shader sources from the Ogre program script, textures from the material script, and also verifies the bundled font, resource locations and checked-in runtime templates. `--root` supports isolated fixtures without touching live assets. | The repository passes 45 checks. An isolated copy with one referenced terrain shader omitted reports its exact path and returns 1. |
...
| B2 | Done | Document and script build verification (old M0.4). | `verify_build.ps1` uses bundled Premake, locates VS2022 MSBuild, rejects OIS demo sources, builds Debug/Release x64 and runs all seven current tests after each build. `verify_build.sh` applies the same contract through Premake/Make on Linux and macOS. README lists one command and its host prerequisites per path. | The Windows wrapper regenerates projects, reports `OIS source inventory valid`, completes both full builds and fourteen test executions with `[BUILD_VERIFY] status=PASS`. This Windows-only session has no Bash runtime, so native Unix execution remains platform-hosted. |
| B3 | Done | Verify the macOS path. | Platform macros and header paths are scoped per host; Ogre core/GLSupport receive their OSX headers, OIS receives Cocoa headers, and the stale WiiMote demo is excluded. The Cocoa GL3Plus option name now matches the tracked `Mine.cfg` `VSync` key and remains registered even when display-mode enumeration is unavailable. `scripts/verify_xcode.sh` uses quiet Xcode builds while preserving per-target logs. | On 2026-08-16 an Apple M1 Pro completed Debug/Release client plus seven-test builds, all 14 test executions (`checks=330 failures=0` in both world runs), two validation-only starts and two real Cocoa 120-frame starts. Both window probes persisted surface contact at `Y=66`; the gate reports `[XCODE_VERIFY] status=PASS` under `build/xcode-validation-20260816185723`. |
...
| B4 | Done | Remove dead build artifacts. | Removed ignored outputs for the deleted `HelloMine3DFoundationRuntimeSmoke` target and the pre-E5 standalone `HelloMine3DOgreBootstrap` target. Both verification wrappers now reject any `HelloMine3D*` executable outside the current client-plus-seven-tests allowlist. | After Debug/Release regeneration and builds, the Windows wrapper reports `executable inventory valid`; `bin/` contains exactly `HelloMine3D.exe` and the seven current test executables. |
...
the macOS-native B3 gate is deferred. These tasks must retain portable core
boundaries even though the current acceptance host is Windows.

| ID | Status | Task | Validation |
| -- | ------ | ---- | ---------- |
| W1 | Done | Add a simple time-of-day and fog parameter channel without rewriting stored sunlight. The portable 24,000-tick model supplies daylight, sky tint, fog colour and fog density without touching any stored block-light or sunlight values. Ogre applies the values to terrain, water, flora, actors, the skybox and scene fog; debug UI and performance captures expose them. `HELLOMINE3D_WORLD_TIME` / `run_render_capture.ps1 -WorldTime` pins a repeatable frame. Environment updates explicitly load every target material first, so a scene with no water, glass or actors still accepts the global parameters. | Eight `W1/*` assertions cover anchors, wrapping, value bounds, twilight continuity, automation override and stats. The Windows Debug/Release gate passes all tests (`checks=255 failures=0`). Same-scene GTX 1050 Ti / OpenGL 4.6 captures at world times 6000 and 18000 pass and are tracked as `validation-day.png` / `validation-night.png`; sampled mean luma changes from 56.096 to 12.993 while the debug panel reports environment light 1.000 / 0.180 (2026-08-13). |
...
| R3 | Verify | Added physical-input protocol v1, a machine-checkable record template and a validator that rejects incomplete metadata, unknown results and false overall passes. Controller, movement, collision, break/place, combat, container conservation, focus isolation, save/relaunch and clean Cocoa shutdown are automated; the ordered physical run is retained only for OIS device/focus mapping. | V2, D2, D4 | Debug/Release pass 330 world assertions, including swept falling collision and stable contact; two Cocoa 120-frame probes stay at `Y=66`. A 2026-08-16 macOS observation closed by the window button exited `0` with full OGRE shutdown and no crash report; InputMethodKit `NSXPCDecoder` messages were system warnings. A person is still required only to certify physical key/mouse/focus delivery on the target Release build. |
...
| R5 | Done | Added a self-contained Windows x64 distribution directory and deterministic ZIP containing the Release client, 42 base resources, runtime templates, optional packs, README and 14 third-party notices. Saves, logs, captures, build products and developer files are excluded. | D6, W2, W3 | Clean-root exact inventory, validation-only startup, a real three-frame Ogre window, missing-shader and stale-extra-resource negatives all pass. The accepted optional-pack distribution contains 61 files; two rebuilds produced ZIP SHA-256 `F4F3C448E75031F30EB788FF72C5F22A6A32CDF6C85A90164D1E16B7F807BB69`. |
...
| X2 | Done | Routed block definitions, shapes, textures, shaders, resource scripts and fonts through one startup-frozen effective view without changing gameplay registration. Unoverridden files fall back to base resources. | X1 | Eighteen resolver assertions and actual no-pack/packed validation-only client runs pass. Missing or invalid sources fail before Ogre construction; effective manifests contain 42 sorted entries and identify `base` or the owning pack. |
...
| -- | ------ | -------- | ---- | ---------- | ------------------- |
| K1 | Todo | P0 | Define a stable world catalogue and metadata contract. Each world has an immutable id plus validated display name, seed, save-format version, creation time, last-played time and last-known build identity. Discover version-1/2 worlds without renaming their directories; reject duplicates, path escape and malformed metadata explicitly. | R3, S2.1, S2.6 | Focused fixtures cover empty/multiple/legacy catalogues, stable ordering, duplicate ids, invalid names/timestamps/versions, symlink or traversal attempts and a missing metadata file. Enumeration never creates or mutates a world. |
| K2 | Todo | P0 | Make world and chunk writes transactional. Write a same-volume temporary generation, durably flush where the host supports it, parse/validate the complete candidate and atomically replace the published file only after validation. Preserve the last valid generation on every failure. | K1, S2.1-S2.6 | Deterministic fault injection before write, mid-write, before flush, before validation and before replace leaves the previous world loadable and quarantines incomplete artifacts. Successful saves preserve all existing compatibility fixtures and expose total/maximum save duration. |
| K3 | Todo | P0 | Add bounded rotating backups, corruption quarantine and verified restore. A restore first copies the chosen backup into a new candidate, validates it, then publishes it through K2; it never overwrites the only backup in place. | K2 | Backup count/bytes stay within configured limits. Corrupt primary, corrupt backup, interrupted restore and old-version restore fixtures have specific diagnostics; at least one valid backup restores player, inventory, chunks, block entities, crops and actors. |
| K4 | Todo | P1 | Add the main-menu world screen: list, create, open, rename, recover and delete. Rename changes only the validated display name. Delete requires confirmation and moves the world to a bounded recoverable area before permanent cleanup. | K1-K3, R3 | Headless command tests prove id/path stability and recoverable deletion. A Release physical-input record covers keyboard/mouse focus, invalid-name feedback, create/open/rename, cancel/confirm delete, restore and return to the menu without world actions leaking through UI focus. |
...
| ID | Status | Priority | Task | Depends on | Required acceptance |
| -- | ------ | -------- | ---- | ---------- | ------------------- |
| H1 | Todo | P1 | Integrate a minimal Windows minidump path, preferring Google Breakpad after an integration audit. Install the handler before Ogre construction, write below a dedicated crash directory outside world saves, bound recursive-handler behaviour and keep upload disabled. | B2, R5 | A controlled Release crash produces exactly one non-empty dump and exits non-zero without corrupting an active world. Ordinary validation-only and real-window starts produce no dump. The build records the selected Breakpad revision/license and keeps portable game code free of Windows exception types. |
...
| ID | Status | Priority | Task | Depends on | Required acceptance |
| -- | ------ | -------- | ---- | ---------- | ------------------- |
| Q1 | Doing | P0 | Define versioned performance scenes, metric schema and approved budgets before new systems change the workload. The schema-v2 portable comparator and macOS launcher now record build/platform/architecture/GPU/driver/VSync/window/FOV/resource-manifest/resource-pack/world/save/storage/residency identity. The accepted pre-stage-8 nominal Release baseline is documented; cold start, world entry, save, restore, fast-streaming and scaled-gameplay contracts still need their Q1 fixtures and approved target-Windows measurements. | R1, R2, D6 | Eight schema-v1/v2 PASS/REGRESSION/INCOMPARABLE/INVALID fixtures pass. Two Apple M1 Pro x86_64/Rosetta samples compare `PASS` in both directions with the unchanged R1 thresholds. Missing schema-v2 identity is `INVALID`, while platform/configuration/residency changes are `INCOMPARABLE`; Q1 cannot close until every named scene has its required fixtures and approved Windows threshold. |
| Q2 | Todo | P0 | Instrument startup phases, catalogue enumeration, world entry, save transaction, backup and restore. Report total plus longest main-thread stall and keep timing collection bounded when disabled. | Q1, K1-K3 | Fixed fixtures emit complete summaries for success and failure paths. The comparator fails non-zero on approved startup/world-entry/save/restore budget regressions, while instrumentation-off results preserve current gameplay state and frame metrics. |
...
| ID | Status | Priority | Task | Depends on | Required acceptance |
| -- | ------ | -------- | ---- | ---------- | ------------------- |
| G1 | Todo | P1 | Add a strict data-driven recipe registry. Recipes use registered material ids, bounded shaped/shapeless inputs and outputs, deterministic duplicate resolution and startup-frozen base-resource lookup; they cannot execute code or introduce runtime reload. Resource-pack v1 does not gain a hidden seventh class: recipe overrides require a separately versioned future contract. | D1, D5, X1-X3 | Parser/registry fixtures cover valid shaped/shapeless recipes, unknown materials, duplicate ids/patterns, invalid counts, stale/missing base resources and manifest ownership. Resource-pack validation rejects unversioned recipe overrides, and no recipe change requires a branch in inventory transfer code. |
| G2 | Todo | P1 | Add player crafting and a workbench container through the normal use/UI boundary. Preview is pure, taking output consumes inputs atomically, shift/repeat behaviour is bounded and closing/reloading cannot duplicate or lose items. | G1, D2, R3 | Conservation and capacity assertions cover success, insufficient ingredients, full inventory, repeated craft, unload/reload and container close. A Release input record opens the workbench, crafts, transfers, closes by button/Escape and proves world actions remain suppressed while focused. |
...
3. **Protect player data** - complete K2-K3 and Q2 with save fault injection, bounded backups, verified restore and startup/save timing gates.
4. **Expose the safe flow** - complete K4 and G1-G2 for world selection, recipes and crafting through the normal UI/input boundary.
...
8. **Deferred host work** - B3 native macOS acceptance is complete. Resume only R4 on a host/configuration that can provide supported ThreadSanitizer evidence; do not let its deferred status block Windows work.
...
| Headless tests | seven `bin\HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries | All code changes |
...
| World catalogue fixtures | K1 test target/harness to be added | World discovery, metadata, names, ids or world-directory changes |
| Transactional save fault injection | K2/K3 harness to be added | Save publication, backup, quarantine, restore or format migration changes |
| Local crash artifact smoke | H1-H3 controlled-crash/symbolization harness to be added | Exception handling, crash context, symbols or packaged crash UX changes |
| Startup/save/restore budget | Q1/Q2 capture and comparator extensions to be added | Catalogue, startup, world-entry, save, backup or recovery changes |
