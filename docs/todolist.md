...
| 玩法数据层 | G1-G6、N1-N5 已完成：目标、熔炉/铁级、食物恢复、差异化敌人/掉落和 terrain v2 路标探索已经串联；当前进入 N6 产品体验。 |
...
| 当前 Windows 门禁 | 2026-08-20 完整通过 46 项资源、36 个性能夹具、R3 模板、VS 工程生成、Debug/Release 0 错误全量重建、十三目标测试、隐藏客户端、后台受控 dump 和 66 文件干净发行包。 |

## 进度摘要

拆分前总账共有 77 个正式任务；当前为 68 个 `Done`、4 个 `Verify`、3 个 `Doing`、
2 个 `Todo`。第 8 阶段的 16 项任务中，K1-K4、G1-G6 和 H1 已完成；H2、Q1、Q2
正在推进；H3、Q3 尚未开始。Alpha 开发检查点、N1-N5 已关闭，当前进入 N6 产品体验。
...
| N6 | Doing | P1 | 统一 UI 信息层级、配方/目标可读性、键位与辅助选项，并收口正式声音反馈。 | 主要界面和输入模型冻结；失败提示可观察；自动 UI/配置门禁通过，真人 R3 记录后置到功能冻结后。 | N5、G4-G5 |
...
| 8：内容扩展（当前 N6） | `N1-N6` | N1-N5 已完成目标、成长、恢复、战斗掉落和生态探索；当前统一 UI、配方/目标可读性、键位辅助与声音反馈。 | N6 不改写已冻结的玩法、生成和存档语义；自动门禁随开发执行，真人 R3 在界面冻结后集中记录。 |
...
熔炉/铁级成长、食物恢复、战斗掉落和生态探索已由 N1-N5 交付；当前 N6 体验批次按
`docs/game-development-roadmap.md` 继续推进，但不回写拆分前 77 项历史总账。
