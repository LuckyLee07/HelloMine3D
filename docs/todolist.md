...
| ID | Status | Priority | Task | Depends on | Required acceptance |
| -- | ------ | -------- | ---- | ---------- | ------------------- |
| Q1 | Doing | P0 | Define versioned performance scenes, metric schema and approved budgets before new systems change the workload. The schema-v2 portable comparator and macOS launcher now record build/platform/architecture/GPU/driver/VSync/window/FOV/resource-manifest/resource-pack/world/save/storage/residency identity. Optional Tracy 0.13.1 live profiling now marks frames, fixed simulation, world/chunk work and mesh generation while remaining default-off. The accepted pre-stage-8 nominal Release baseline is documented; cold start, world entry, save, restore, fast-streaming and scaled-gameplay contracts still need their Q1 fixtures and approved target-Windows measurements. | R1, R2, D6 | Eight schema-v1/v2 PASS/REGRESSION/INCOMPARABLE/INVALID fixtures pass. Two Apple M1 Pro x86_64/Rosetta samples compare `PASS` in both directions with the unchanged R1 thresholds. Missing schema-v2 identity is `INVALID`, while platform/configuration/residency changes are `INCOMPARABLE`; Q1 cannot close until every named scene has its required fixtures and approved Windows threshold. |
