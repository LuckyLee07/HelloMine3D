...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 239 assertions, including configured-seed determinism, generated runtime-config ownership, strict block-definition and shape parsing, block-use events, behavior dispatch and metadata-backed behavior, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, resource-driven non-cube shapes, deterministic cave generation, load-order-independent cross-chunk structures, mob chase and damage immunity, sunlight/block-light storage, local relighting and transparent-pass rules, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=239 failures=0` |
...
| B3 | Blocked | Verify the macOS path. | Platform macros and header paths are now scoped per host; Ogre core/GLSupport receive their OSX headers, OIS receives Cocoa headers, and the stale WiiMote demo is excluded. `tools/validate_xcode_generation.ps1` generates all 22 Xcode projects and runs 123 checks over targets, dependencies, frameworks, Cocoa/OIS/OSX sources, header paths, foreign-platform leakage and the native verifier contract. `scripts/verify_xcode.sh` is the one-command native gate. | Static generation passes. Final acceptance requires `bash scripts/verify_xcode.sh` on macOS; it builds Debug/Release, executes five tests per configuration, then runs validation-only and real-window three-frame client probes. |
