...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 209 assertions, including configured-seed determinism, generated runtime-config ownership, strict block-definition diagnostics, block-use events, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, sunlight/block-light storage and local relighting, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=209 failures=0` |
...
| Performance baseline | Ogre frame/tick/counter collection is wired and the script targets the sole client. The 2026-08-12 L3 hardware baseline records 60.09 FPS, 17.58 ms frame P95 and no frames over 33 ms. | `docs/performance-baseline.md` |
...
Goal: visual quality, after the mesh pipeline is stable. Sunlight and block
light storage are now present; edit-time relighting and refined transparency
follow.
