...
| 玩法数据层 | G1-G6、N1-N6 与 N7A/N7B 已完成：terrain v2 路标现在可用 2 个铁锭激活，触发两波有界守卫战、一次性奖励和可关闭的胜利覆盖层；下一批为 N8A 战斗可读性。 |
...
| 当前 Windows 门禁 | 2026-08-25 完整通过 49 项资源、36 个性能夹具、R3 `NOT_RUN` 模板、VS 工程生成、Debug/Release 0 错误全量重建、十三目标测试、隐藏客户端、131,817 字节后台受控 dump 和 69 文件干净发行包；ZIP SHA-256 为 `9F52C3C9F6F9DE60F85D7864ED707A60FD358B15E062CF9E85807ABA8494BDD2`。 |

## 进度摘要

拆分前总账共有 77 个正式任务；历史总账仍为 73 个 `Done`、4 个 `Verify`。新增的 Stage 9
批次不回写该统计：`FS1-FS3` 与 `N7A/N7B` 当前为 `Done`，`N8A-N12` 保持 `Planned`。
R3 已做部分非正式真人自测，尚未形成十二项正式记录，其余按用户决定延后且不阻塞后续玩法开发。

| 类型 | 数量 | 任务 |
| ---- | ---- | ---- |
| 已完成的第 8 阶段正式任务 | 16 | K1-K4、G1-G6、H1-H3、Q1-Q3 |
| Stage 9 已完成 | 5 | FS1 首次进入正确性、FS2 天空/水面/颜色、FS3 体素美术/HUD、N7A 结局状态与文本键基础、N7B 路标胜利闭环 |
...
下列批次已经落成可回归路线，`FS1-FS3` 与 `N7A/N7B` 已转成 `Done`，其余保持 `Planned`；它们不计入拆分前
