...
批次不回写该统计：`FS1-FS3` 当前均为 `Done`，N7-N12 保持 `Planned`。Release
Candidate 的自动化证据已经闭环，R3 延后到首屏和 HUD 稳定后执行。

| 类型 | 数量 | 任务 |
| ---- | ---- | ---- |
| 已完成的第 8 阶段正式任务 | 16 | K1-K4、G1-G6、H1-H3、Q1-Q3 |
| Stage 9 已完成 | 3 | FS1 首次进入正确性、FS2 天空/水面/颜色、FS3 体素美术/HUD |
...
下列批次已经落成可回归路线，`FS1-FS3` 已转成 `Done`，其余保持 `Planned`；它们不计入拆分前
...
| RC0 基线收口 | Planned | P0 | FS3 后验证未推送提交 bundle，完成 R3，并关闭 D2/D4/D6。 | bundle、文档、R3 与本地 RC 标签边界全部确认。 |
...
预排执行顺序为 `FS1 -> FS2 -> FS3 -> RC0 -> N7A -> N7B -> N8A -> N8B -> N9A -> N9B -> N10 -> N11A ->
