...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 266 assertions, including deterministic time-of-day values, terrain-buffer layout metrics, configured-seed determinism, generated runtime-config ownership, strict block-definition and shape parsing, block-use events, behavior dispatch, metadata-backed behavior and bounded random ticks, stateful-block ownership and reload, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, resource-driven non-cube shapes, deterministic cave generation, load-order-independent cross-chunk structures, mob chase and damage immunity, sunlight/block-light storage, local relighting and transparent-pass rules, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=266 failures=0` |
...
| Active next scope | The Windows sandbox foundation, R1 and D1 are closed. D2 is next, followed by the remaining D playable-loop milestones, R hardening/release work and the bounded X resource-pack layer. B3 and R4 remain host-deferred. | Milestones D, R and X below |
