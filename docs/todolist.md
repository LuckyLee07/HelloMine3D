...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 200 assertions, including configured-seed determinism, generated runtime-config ownership, strict block-definition diagnostics, block-use events, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, sunlight and block-light storage, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=200 failures=0` |
...
| Performance baseline | Ogre frame/tick/counter collection is wired and the script targets the sole client. The 2026-08-12 L2 hardware baseline records 60.14 FPS, 17.69 ms frame P95 and no frames over 33 ms. | `docs/performance-baseline.md` |
