...
| 玩法数据层 | G1-G6、N1-N6、N7A-N12C、BETA-RC 工程封板、V10A 与 V10B1 均已完成；V10B2 工程实现完成并等待项目所有者自行启动前台 Release 做视觉关闭。 |
...
当前建立 8 个视觉批次，其中 V10A/V10B1 为 `Done`、V10B2 为 `Implemented（前台视觉 Verify）`、其余为 `Planned`；
V10B1 的 macOS shader 子状态因当前无目标机器保持 `Verify`。
...
| Stage 10 已完成 | 2 | V10A 顶点平滑光照/AO、V10B1 材质/图集管线 |
| Stage 10 待关闭/开发 | 6 | V10B2 原创材质资产仅剩前台视觉 Verify；V10B3 生态着色/确定性变体、V10C 定向大气/立体云、V10D 可选方向阴影、V10E 轻量后处理、VISUAL-RC 视觉封板待开发 |
...
| V10B2 原创材质资产 | Implemented（前台视觉 Verify） | P0 | 37 个原创语义 tile、top/side/bottom、四类 Alpha、双语 layout、确定性生成与许可已落地；HUD/手持改用冻结 atlas profile。 | 自动合同 106/106、资源包 56/56、Release 世界 718/718、13 类启动负例、67 项 manifest、3 张隐藏固定图和 85 文件干净包通过；项目所有者仍需自行启动前台 Release 连续观察 5-10 分钟，合同见 `docs/material-visual-contract-v1.md`。 |
...
当前 V10B2 工程实现已经完成，但按焦点安全约定不由自动化打开可见窗口；项目所有者关闭前台
视觉 `Verify` 后才转入 V10B3。以上新增批次仍不回写拆分前 77 项历史总账。
