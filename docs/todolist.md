...
| Test targets | Seven first-party headless targets cover coordinates, mesh dirty propagation, save compatibility, actor lifecycle, the 338-check world stack, deterministic soak scheduling and resource-pack resolution. | The seven `HelloMine3D*Tests.exe` / `*Smoke.exe` / `*Soak.exe` binaries listed in `scripts\verify_build.ps1` |
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 338 assertions. Its D6 scenario chains normal gameplay actions after one fixed fixture; V2 additionally covers swept collision, held-input semantics, normalized movement, fixed-tick sampling and camera interpolation. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=338 failures=0` |
...
| B3 | Done | Verify the macOS path. | Platform macros and header paths are scoped per host; Ogre core/GLSupport receive their OSX headers, OIS receives Cocoa headers, and the stale WiiMote demo is excluded. The Cocoa GL3Plus option name now matches the tracked `Mine.cfg` `VSync` key and remains registered even when display-mode enumeration is unavailable. `scripts/verify_xcode.sh` uses quiet Xcode builds while preserving per-target logs. | On 2026-08-16 an Apple M1 Pro completed Debug/Release client plus seven-test builds, all 14 test executions (`checks=338 failures=0` in both world runs), two validation-only starts and two real Cocoa 120-frame starts. Both window probes persisted surface contact at `Y=66`; the gate reports `[XCODE_VERIFY] status=PASS` under `build/xcode-validation-20260816195009`. |
