...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 327 assertions. Its D6 scenario chains normal gameplay actions after one fixed fixture: seed acquisition, planting/growth/harvest, chest transfer, natural-mob combat and loot pickup, replanting, save and relaunch. Earlier chunk, render-data, lighting, persistence, actor, interaction and generation coverage remains in the same gate. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=327 failures=0` |
...
| Active next scope | All 13 selected D/R/X implementation tasks are now landed locally. D1, D3, D5, R1, R2, R5 and X1-X3 are closed; D2, D4 and D6 have complete code, automated coverage and hardware evidence but remain `Verify` until a human completes the R3 physical-input record. B3 and R4 remain host-deferred. | Milestones D, R and X below |
...
| B2 | Done | Document and script build verification (old M0.4). | `verify_build.ps1` uses bundled Premake, locates VS2022 MSBuild, rejects OIS demo sources, builds Debug/Release x64 and runs all five tests after each build. `verify_build.sh` applies the same contract through Premake/Make on Linux and macOS. README lists one command and its host prerequisites per path. | The Windows wrapper regenerates projects, reports `OIS source inventory valid`, completes both full builds and ten test executions with `[BUILD_VERIFY] status=PASS`. This Windows-only session has no Bash runtime, so native Unix execution remains platform-hosted. |
| B3 | Deferred | Verify the macOS path. | Platform macros and header paths are scoped per host; Ogre core/GLSupport receive their OSX headers, OIS receives Cocoa headers, and the stale WiiMote demo is excluded. `tools/validate_xcode_generation.ps1` generates all 24 Xcode projects and runs 133 checks over targets, dependencies, frameworks, Cocoa/OIS/OSX sources, header paths, foreign-platform leakage and the native verifier contract. `scripts/verify_xcode.sh` remains the one-command native gate. | Windows-side static generation passes. Native acceptance is intentionally deferred while Windows iterations continue; resume by running `bash scripts/verify_xcode.sh` on macOS. |
...
| R3 | Verify | Added physical-input protocol v1, a machine-checkable record template and a validator that rejects incomplete metadata, unknown results and false overall passes. The ordered run covers focus recovery, WASD/sprint, mouse look and its `L` toggle, flight/sneak/vertical controls, number and physical-wheel hotbar selection, break/attack/place, chest use/transfers, UI action isolation and both close paths. OIS wheel delta is now routed through the bounded hotbar-selection path. | V2, D2, D4 | The tracked `NOT_RUN` template passes schema validation in the Windows gate. A human must still execute `docs/manual-input-acceptance-v1.md` on the target Release build and validate the resulting record with `-RequirePass`; automation remains non-intrusive and does not synthesize desktop input. |
...
The remaining acceptance and deferred host work is:

1. **R3** - execute protocol v1 with a person on the target Release build; this also closes D2, D4 and D6.
2. **B3 and R4** - resume only on hosts that can provide native macOS and ThreadSanitizer evidence.
3. After those gates, select a new product milestone rather than silently expanding resource packs into scripts, hot reload or network content.
