...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 119 assertions covering S0-S6, fixed ticks, debug-panel startup options, block selection, synthetic player input, height-map edits, loader-thread stress, headless atlas coordinates, and the mesh halo snapshot. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=119 failures=0` |
