...
| 玩法数据层 | G1-G6、N1-N6、N7A/N7B 与 N8A/N8B 已完成；当前下一批为 N9A 确定性结构框架。 |
...
| 当前 Windows 门禁 | 2026-08-25 N8B 完整通过 49 项资源、36 个性能夹具、R3 部分自测/其余延期记录、VS 工程生成、Debug/Release 0 错误全量重建、十三目标测试、隐藏客户端、129,201 字节后台受控 dump 和 69 文件干净发行包；ZIP SHA-256 为 `724D1C06CF32EF3805131F814167ECB9296392A8228520D5B45AD79FD308FBFA`。 |

## 进度摘要

拆分前总账共有 77 个正式任务；历史总账仍为 73 个 `Done`、4 个 `Verify`。新增的 Stage 9
批次不回写该统计：`FS1-FS3`、`N7A/N7B` 与 `N8A/N8B` 当前为 `Done`，`N9A-N12` 保持 `Planned`。
R3 已做部分非正式真人自测，尚未形成十二项正式记录，其余按用户决定延后且不阻塞后续玩法开发。

| 类型 | 数量 | 任务 |
| ---- | ---- | ---- |
| 已完成的第 8 阶段正式任务 | 16 | K1-K4、G1-G6、H1-H3、Q1-Q3 |
| Stage 9 已完成 | 7 | FS1 首次进入正确性、FS2 天空/水面/颜色、FS3 体素美术/HUD、N7A 结局状态与文本键基础、N7B 路标胜利闭环、N8A 战斗可读性、N8B 远程敌人与投射物 |
...
下列批次已经落成可回归路线，`FS1-FS3`、`N7A/N7B` 与 `N8A/N8B` 已转成 `Done`，其余保持 `Planned`；它们不计入拆分前
