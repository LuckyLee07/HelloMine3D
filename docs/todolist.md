...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 192 assertions, including configured-seed determinism, generated runtime-config ownership, strict block-definition diagnostics, block-use events, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, sunlight storage, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=192 failures=0` |
| Render smoke | Ogre terrain, water, flora, outline and HUD are implemented and validation-only startup passes. GTX 1050 Ti / OpenGL 4.6 hardware captures now exercise the assembled client, including the L1 sunlight scene. | `docs/render-regression-smoke.md` |
| Performance baseline | Ogre frame/tick/counter collection is wired and the script targets the sole client. The 2026-08-12 L1 hardware baseline records 60.14 FPS, 17.75 ms frame P95 and no frames over 33 ms. | `docs/performance-baseline.md` |
...
Goal: visual quality, after the mesh pipeline is stable. Sunlight storage is
now present; block light, edit-time relighting and refined transparency follow.
