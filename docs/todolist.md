...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 236 assertions, including configured-seed determinism, generated runtime-config ownership, strict block-definition and shape parsing, block-use events, behavior dispatch and metadata-backed behavior, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, resource-driven non-cube shapes, deterministic cave generation, load-order-independent cross-chunk structures, sunlight/block-light storage, local relighting and transparent-pass rules, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=236 failures=0` |
