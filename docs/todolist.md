...
| ---- | ------------- | ------------ |
| Build system | Premake is the sole project generator. `vs2022.bat` generates the six first-party targets plus the E1 Ogre/OIS static-library graph for VS2022 x64. | `vs2022.bat`, `MSBuild build\HelloMine3D.sln /p:Configuration=Debug /p:Platform=x64` |
| Dependencies | SFML 3 remains the active client path while Ogre 1.10, GL3Plus, OIS and the minimum image/font/archive dependency closure build from vendored source. No checked-in binaries. | Debug and Release full-solution builds |
