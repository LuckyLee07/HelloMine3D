...
当前唯一计划开发的批次是 P11A 核心操作手感；P11B-P11F 只按固定顺序排队，不同时开工。
...
| `docs/playability-experience-roadmap.md` | Stage 11 的 P11A-P11F 可玩性与操作体验顺序、版本预案、Physical Input v2 和 PLAYABILITY-RC 门禁。 |
...
| 玩法数据层 | G1-G6、N1-N6、N7A-N12C、BETA-RC 与 Stage 10/VISUAL-RC 均已完成 Windows 工程范围；Stage 11 已选定，P11A 是下一开发批次。 |
...
D4/D6 和 Stage 9/10 的全部人工体验。Stage 11 新增六个不回写历史统计的批次：P11A 为
`Planned`，P11B-P11F 为 `Queued`。P11A 必须建立 Physical Input v2，并与视觉/双语/听感
验收分开；不能因代码完成而自动关闭 D2/D4/D6/R3。
...
| Stage 11 下一批 | 1 | P11A 核心操作手感（Planned） |
| Stage 11 已排队 | 5 | P11B 动作反馈、P11C 前 30 分钟、P11D 探索奖励、P11E 敌人表现、P11F PLAYABILITY-RC |
...
Stage 11 的固定顺序为 `P11A -> P11B -> P11C -> P11D -> P11E -> P11F`。当前只开放
P11A；后续批次必须在前一批通过真实试玩退出条件后，逐个从 `Queued` 切换为 `Planned`。
权威范围、版本预案、试玩记录字段和非目标见 `docs/playability-experience-roadmap.md`。

| 批次 | 状态 | 优先级 | 计划交付 | 退出边界 |
| ---- | ---- | ------ | -------- | -------- |
| P11A 核心操作手感 | Planned | P0 | 分离使用/放置/格挡，扩展鼠标重绑定，统一灵敏度增量与焦点/捕获恢复，支持冲刺/潜行按住或切换。 | Physical Input v2 在目标 Windows 机器真实覆盖移动、视角、世界动作、UI、焦点与重启；自动合同、VS2017/v141 双配置和受影响回归通过；无串键、焦点泄漏或分辨率/帧率相关漂移。 |
...
| P11E 敌人表现与战斗层次 | Queued | P1 | 多部件体素轮廓、完整关键动作与独特 Waystone 守护阶段。 | 不依赖名称即可辨识主要敌人和攻击窗口；新增敌人必须带来新决策与新奖励。 |
...
产品方向评审已选择 Stage 11，下一开发批次为 P11A；macOS 原生与正式产品体验保持 Verify。
以上新增批次
仍不回写拆分前 77 项历史总账。
