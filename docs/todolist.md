...
| Runtime validation | `HelloMine3DWorldRuntimeSmoke` drives the real world/actor stack through 180 assertions, including generated runtime-config ownership, strict block-definition diagnostics, block-use events, frustum-priority ordering, enclosed-section skipping, opaque greedy meshing, bounded dirty-section processing, mesh build metrics, versioned renderer mesh snapshots, stale-upload rejection, actor render snapshots and actor persistence. | `bin\HelloMine3DWorldRuntimeSmoke.exe` -> `checks=180 failures=0` |
