...
| 所有者 | 字段 | 设置页权限 |
| ------ | ---- | ---------- |
| `UserSettings` | 窗口宽高、全屏、视距、方向阴影档位、FOV、鼠标灵敏度、反转 Y、主/UI/效果/环境/音乐音量、UI 缩放、语言、声音字幕、操作提示和九项玩法键位 | 可编辑 |
...
5. 成功后才更新内存配置。视觉/逻辑相机 FOV、灵敏度、反转 Y、视距、方向阴影档位、音量、UI 缩放、辅助选项和键位立即生效；窗口尺寸和全屏
   保存成功但明确提示重启生效。
...
`bin/config.txt` 使用严格文本格式，V10D 后当前版本为 5：
...
- 无 `settings_version` 的旧文件按 legacy v0 读取；显式版本 1/2/3/4 仍按各自字段边界读取。
  缺失字段使用默认值，成功校验后立即原子迁移到 v5；v0-v2 的 locale 固定迁移为 `en-US`，
  v0-v3 的音乐音量固定迁移为默认 0.65。版本 1 混入版本 2 字段、v0-v2 混入 v3 `locale`，
  v0-v3 混入 v4 `musicvolume`，或 v0-v4 混入 v5 `directionalshadowquality` 都拒绝。
- v0-v4 的方向阴影固定迁移为 `off`；v5 只接受 `off`、`medium`、`high`，未知档位严格拒绝。
- 未知版本明确拒绝，不尝试猜测或降级。
...
- 缺失文件生成 v5、默认关闭方向阴影、五类音量、语言、辅助选项和九项键位默认值；
- legacy v0、版本 1/2/3/4 自动迁移，旧版本越界字段、未知阴影档位和未知版本拒绝；
