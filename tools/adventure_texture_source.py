...
def tiles(edge=32):
    ...
    for index, name in enumerate(NAMES):
        ...
        if name in OVERRIDE_SOURCES:
            authored = Image.open(OVERRIDE_SOURCES[name]).convert('RGBA')
            if authored.width != authored.height:
                raise ValueError(f'Adventure material override must be square: {name}')
            tile = authored.resize((edge, edge), Image.Resampling.NEAREST)
        pixels = []
