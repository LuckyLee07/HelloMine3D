#!/usr/bin/env python3
"""Compare E3's frozen v8/v9 meadow Release capture protocol."""
...
GROUPS = (
    ('meadow-steady', 'meadow', False, '1536 120 800'),
    ('meadow-streaming', 'meadow', True, '1536 120 800'),
)
ROUND_ORDER = ((8, 9), (9, 8), (8, 9))
...
def compare(root):
    ...
    for name, scene, streaming, position in GROUPS:
        ...
        if any(right['started_unix'] <= left['started_unix']
            ...
            raise ValueError(f'{name}: capture order differs from E3 protocol')
        metrics = {}
        for key in ('frame_p95_ms', 'frame_p99_ms'):
            baseline = [run[key] for run in versions[8]]
            candidate = [run[key] for run in versions[9]]
    ...
    return {
        'schema': 1,
        'source': 'E3_RELEASE_CLIENT_CAPTURE',
        'guardrail': 'three-run median v9/v8 P95 and P99 <= 1.10 per group',
